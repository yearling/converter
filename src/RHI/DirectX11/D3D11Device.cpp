...
bool D3D11Device::CreateSwapChain(void* wnd) {
    ...
    //chain_des.BufferDesc.Width = width_;
    //chain_des.BufferDesc.Height = height_;
    ...
    //chain_des.SampleDesc = sample_desc;
    //chain_des.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
    //chain_des.BufferCount = 2;
    //chain_des.OutputWindow = *(reinterpret_cast<HWND*>(wnd));
...
#
  ...
  /*  d3d_swap_chain_->GetBuffer(1, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&back_buffer));
    hr = d3d_device_->CreateRenderTargetView(back_buffer, nullptr, &main_RTVs_[1]);
    if (FAILED(hr)) {
        ERROR_INFO("Create render target view failed!");
        return false;
    }*/
    ...
    D3D11_TEXTURE2D_DESC depth_stencil_des = {0};
    depth_stencil_des.Width = width_;
    depth_stencil_des.Height = height_;
    depth_stencil_des.MipLevels = 1;
    depth_stencil_des.ArraySize = 1;
    depth_stencil_des.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
    depth_stencil_des.SampleDesc.Count = 1;
    depth_stencil_des.SampleDesc.Quality = 0;
    depth_stencil_des.Usage = D3D11_USAGE_DEFAULT;
    depth_stencil_des.BindFlags = D3D11_BIND_DEPTH_STENCIL;
    depth_stencil_des.CPUAccessFlags = 0;
    depth_stencil_des.MiscFlags = 0;

    hr = d3d_device_->CreateTexture2D(&depth_stencil_des, nullptr, &main_depth_buffer_);
    if (FAILED(hr)) {
        ERROR_INFO("Create depth stencil buffer failed!");
        return false;
    }

    // Create DSV
    hr = d3d_device_->CreateDepthStencilView(main_depth_buffer_, nullptr, &main_DSV_);
    if (FAILED(hr)) {
        ERROR_INFO("Create depth stencil view failed!");
        return false;
    }
...
bool D3D11Device::OnResize(int width, int height) {
    ...
    height_ = height;
    HRESULT hr = S_OK;
    if (FAILED(hr = d3d_swap_chain_->ResizeBuffers(1, width_, height_, DXGI_FORMAT_R8G8B8A8_UNORM, 0))) {
   ...
   /*  if (FAILED(hr = d3d_swap_chain_->GetBuffer(1, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&back_buffer)))) {
        ERROR_INFO("d3d swap chain GetBuffer failed!");
    }*/
    /*if (FAILED(hr = d3d_device_->CreateRenderTargetView(back_buffer, 0, &main_RTVs_[1]))) {
        ERROR_INFO("d3d device  create render target view failed!");
    }*/
    // CreateTexture


    Create2DTextureDSV_SRV(width_, height_, DXGI_FORMAT_D24_UNORM_S8_UINT, main_depth_buffer_, "m_DepthStencilTexture");
