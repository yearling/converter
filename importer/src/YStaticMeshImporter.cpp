...
bool YFbxImporter::BuildStaticMeshFromGeometry(FbxNode* node, YLODMesh* raw_mesh, std::vector<std::shared_ptr<YFbxMaterial>>& existing_materials)
{
	...
	if (material_count == 0)
	{
		existing_materials.push_back(std::make_shared<YFbxMaterial>());
		material_count = 1;
	}
	...
	std::unordered_map<uint64_t, int> map_control_point_to_edge;
	...
	for (int fbx_edge_index = 0; fbx_edge_index < fbx_edge_count; ++fbx_edge_index)
	{
		...
		int edge_vertex_start = vertex_offset + edge_start_control_point_index;
		int edge_vertex_end = vertex_offset + edge_end_control_point_index;
		uint64_t compacted_key = static_cast<uint64_t>(edge_vertex_start) << 32 | static_cast<uint64_t>(edge_vertex_end);
		map_control_point_to_edge[compacted_key] = fbx_edge_index;
		uint64_t compacted_key_reverse = static_cast<uint64_t>(edge_vertex_end) << 32 | static_cast<uint64_t>(edge_vertex_start);
		map_control_point_to_edge[compacted_key_reverse] = fbx_edge_index;
	}
	...
	// Compute and reserve memory to be used for vertex instances
	{
		...
		for (int polygon_index = 0; polygon_index < polygon_count; ++polygon_index)
		{
			...
			for (int corner_index = 0; corner_index < polygon_vertex_count; ++corner_index, ++current_vertex_instace_index)
			{
                ...
                YMeshVertexWedge& cur_vertex_instance = raw_mesh->vertex_instances[vertex_instance_index_in_all_mesh];
				...
				for (int uv_layer_index = 0; uv_layer_index < fbx_uvs.unique_count; ++uv_layer_index)
				{
					...
					cur_vertex_instance.uvs[uv_layer_index] = final_uv_vector;
				}
				...
				if (vertex_color)
				{
					...
					cur_vertex_instance.color = YVector4((float)vertex_color_value.mRed, (float)vertex_color_value.mGreen, (float)vertex_color_value.mBlue, (float)vertex_color_value.mAlpha);
				}
				...
				if (normal_layer)
				{
					...
					cur_vertex_instance.normal = tangent_z;
					...
					if (has_NTB_information)
					{
						...
						cur_vertex_instance.tangent = tangent_x;
			...
			{
			    std::shared_ptr<YFbxMaterial> material = existing_materials[real_material_index_in_all_mesh];
				...
				{
					if (raw_mesh->polygon_group_to_material_name.count(group_id))
					{
				...
				if (existing_polygon_group_id == INVALID_ID)
				{
					...
					raw_mesh->polygon_group_to_material_name[existing_polygon_group_id] = material;
				}
			...
			// create polygon edeges
			{
				...
				for (uint32_t polygon_edge_number = 0; polygon_edge_number < (uint32_t)polygon_vertex_count; ++polygon_edge_number)
				{
					...
					if (map_control_point_to_edge.count(compacted_key))
					{
						fbx_edge_index = map_control_point_to_edge[compacted_key];
					}
					else
					{
    ...
    // Compute and reserve memory to be used for vertex instances
    {
        ...
        for (int polygon_index = 0; polygon_index < polygon_count; ++polygon_index)
        {
            ...
            for (int triangle_cornor_index_0_1_2 = 0; triangle_cornor_index_0_1_2 < polygon_vertex_count_3; ++triangle_cornor_index_0_1_2, ++current_wedge_index)
            {
                ...
                const int control_point_index = fbx_mesh->GetPolygonVertex(polygon_index, triangle_cornor_index_0_1_2);
            ...
            //Create a polygon with the 3 vertex instances Add it to the material group
            if (!raw_mesh.material_to_polygon_group.count(material_index))
            {
                ...
                for (int group_id = 0; group_id < raw_mesh.polygon_groups.size(); ++group_id)
                {
                    if (raw_mesh.polygon_group_to_material.count(group_id))
                    {
                        existing_polygon_group_id = group_id;
                        break;
                    }
                ...
                if (existing_polygon_group_id == INVALID_ID)
                {
                    ...
                    existing_polygon_group_id = (int)raw_mesh.polygon_groups.size();
                    raw_mesh.polygon_groups.push_back(tmp_group);
                    raw_mesh.polygon_group_to_material[existing_polygon_group_id] = material;
                }
