	...
	, RasterHeight(0)
{
	...
	Bits.resize(Pitch * Height + 1,0); // alloc +1 to avoid buffer overrun

	Rows.resize( Height );
	SortedRowsIndex.resize(Height);
	...
	if (LayoutVersion >= ELightmapUVVersion::Segments2D)
	{
		...
		SortedColumnsIndex.resize(Width);
...
void FAllocator2D::Clear()
{
	...
	memset(Bits.data(),0, Pitch * RasterHeight *sizeof(uint64));

	RasterWidth  = 0;
	RasterHeight = 0;
}

void FAllocator2D::CopyRuns(std::vector<FRun>& Runs, const std::vector<FRun>& OtherRuns, int32 MaxSize)
{
	Runs.resize(OtherRuns.size());

	MaxSize = YMath::Min((int)Runs.size(), MaxSize);

	FRun* RunsPtr = Runs.data();
	const FRun* OtherRunsPtr = OtherRuns.data();
...
FAllocator2D& FAllocator2D::operator = (const FAllocator2D & Other)
{
	...
	memcpy(Bits.data(), Other.Bits.data(), Pitch * Other.RasterHeight * sizeof(uint64));

	// Zero out anything that was used and not overwritten by the copy
	if (RasterHeight > Other.RasterHeight)
	{
		//FPlatformMemory::Memzero(Bits.data() + Pitch * Other.RasterHeight, Pitch * (RasterHeight - Other.RasterHeight) * Bits.GetTypeSize());
		memset(Bits.data() + Pitch * Other.RasterHeight,0, Pitch * (RasterHeight - Other.RasterHeight) * sizeof(uint64));
	}
...
std::string FAllocator2D::ToString() const
{
	...
	{
		if ((Mode == EMode::FreeSegments && Row.Segments.size() > 1) ||
			(Mode == EMode::UsedSegments && Row.Segments.size() != 0))
		{
			lastRow++;
		}
	}
...
bool FAllocator2D::FindWithSegments( FRect& Rect, const FAllocator2D& Other, std::function<bool (const FRect&)> IsBestRect) const
{
	FRect TestRect = Rect;
...
bool FAllocator2D::TestAllRows( const FRect& Rect, const FAllocator2D& Other, uint32& OutFailedLength ) const
{
	for ( uint32 Index = 0; Index < Other.RasterHeight; ++Index)
	{
		...
		const uint16 y = Other.SortedRowsIndex.data()[Index];

		const FRun& ThisRow  = Rows.data()[ Rect.Y + y ];
		const FRun& OtherRow = Other.Rows.data()[ y ];
...
bool FAllocator2D::TestAllColumns( const FRect& Rect, const FAllocator2D& Other, uint32& OutFailedLength ) const
{
	for ( uint32 Index = 0; Index < Other.RasterWidth; ++Index)
	{
		...
		const uint16 x = Other.SortedColumnsIndex.data()[Index];

		const FRun& ThisColumn  = Columns.data()[ Rect.X + x ];
		const FRun& OtherColumn = Other.Columns.data()[ x ];
...
bool FAllocator2D::TestOneRun(const FRun& ThisRun, const FRun& OtherRun, uint32 RectOffset, uint32 RectLength, uint32 PrimaryResolution, uint32& OutFailedLength) const
{
	...
	const FSegment* FreeSegments      = ThisRun.Segments.data();
	const FSegment* FreeSegmentsEnd   = ThisRun.Segments.data() + ThisRun.Segments.size();
	const uint16*   FreeSegmentLookup = ThisRun.Segments.size() > LoopkupThreshold ? ThisRun.FreeSegmentsLookup.data() : nullptr;
	const FSegment* UsedSegments      = OtherRun.Segments.data();
	const FSegment* UsedSegmentsEnd   = OtherRun.Segments.data() + OtherRun.Segments.size();
	...
	for ( ; UsedSegment < UsedSegmentsEnd; ++UsedSegment)
	{
		...
		const uint32 EndPos   = RectOffset + YMath::Min( UsedSegment->StartPos + UsedSegment->Length, RectLength );
...
void FAllocator2D::InitRuns(std::vector<FRun>& Runs, uint32 PrimaryResolution, uint32 PerpendicularRasterSize)
{
	if (Mode == EMode::FreeSegments)
	{
		...
		{
			Run.Segments.clear();
			Run.Segments.push_back( FreeSegment );
			Run.LongestSegment = PrimaryResolution;
			Run.FreeSegmentsLookup.clear();
		}
	}
	else
	{
...
void FAllocator2D::CreateUsedSegments()
{
	...
	SortedRowsIndex.resize(RasterHeight);

	uint64* BitsData = Bits.data();
    ...
    std::sort(SortedRowsIndex.begin(), SortedRowsIndex.end(), [this](uint32 a, uint32 b) {
        return Rows[b].LongestSegment < Rows[a].LongestSegment;
        });

	// Create segments along the Y axis for each columns
	if (LayoutVersion >= ELightmapUVVersion::Segments2D)
	{
		SortedColumnsIndex.resize(RasterWidth);
        ...
        std::sort(SortedColumnsIndex.begin(), SortedColumnsIndex.end(), [this](uint32 a, uint32 b) {
                return Columns[b].LongestSegment < Columns[a].LongestSegment;
            });
	}
}
...
void FAllocator2D::AddUsedSegment( FRun& Row, uint32 StartPos, uint32 Length )
{
	...
	Row.Segments.push_back(UsedSegment);
...
void FAllocator2D::MergeRun(FRun& ThisRun, const FRun& OtherRun, uint32 RectOffset, uint32 RectLength, uint32 PrimaryResolution, uint32 PerpendicularResolution)
{
	...
	{
		for ( int32 Index = 0; Index < ThisRun.Segments.size(); ++Index )
		{
				...
				StartPos < ThisFreeSegment.StartPos + ThisFreeSegment.Length )
			{
				if ( ThisFreeSegment.Length == 1 )
				{
                    ...
                    ThisRun.Segments.erase(ThisRun.Segments.begin()+Index);
					break;
				}
				...
				uint32 EndPos = RectOffset + YMath::Min( OtherUsedSegment.StartPos + OtherUsedSegment.Length, RectLength ) - 1;
				...
				{
					ThisRun.Segments.push_back( FirstSegment );
				}

				if ( SecondSegment.Length > 0 )
				{
					ThisRun.Segments.push_back( SecondSegment );
				}
    ...
    std::sort(ThisRun.Segments.begin(), ThisRun.Segments.end());
	// When we're dealing with an Axis, the lookup table is the size of the perpendicular Axis
	ThisRun.FreeSegmentsLookup.resize(PerpendicularResolution);
	ThisRun.LongestSegment = 0;

	uint32 LastIndex = 0;

	// Avoid RangeCheck by indexing the pointer directly since we're in a hot loop
	FSegment* SegmentsPtr           = ThisRun.Segments.data();
	uint16*   FreeSegmentsLookupPtr = ThisRun.FreeSegmentsLookup.data();

	int32 SegmentIndex = 0;
	for ( ; SegmentIndex < ThisRun.Segments.size(); ++SegmentIndex )
	{
...
void FAllocator2D::PublishStats(int32 ChartIndex, int32 Orientation, bool bFound, const FRect& Rect, const FRect& BestRect, const FMD5Hash& ChartMD5, std::function<bool (const FAllocator2D::FRect&)> IsBestRect)
{
