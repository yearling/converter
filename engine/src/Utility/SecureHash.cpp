...
void FMD5::Update( const uint8_t* input, int32_t inputLen )
{
	int32_t i, index, partLen;

	// Compute number of bytes mod 64.
	index = (int32_t)((Context.count[0] >> 3) & 0x3F);
...
void FMD5::Final( uint8_t* digest )
{
	uint8_t bits[8];
...
void FMD5::Transform( uint32_t* state, const uint8_t* block )
{
...
void FMD5::Encode( uint8_t* output, const uint32_t* input, int32_t len )
{
	int32_t i, j;
...
void FMD5::Decode( uint32_t* output, const uint8_t* input, int32_t len )
{
	int32_t i, j;
...
std::string LexToString(const FMD5Hash& Hash)
{
	if (!Hash.bIsValid)
	{
		return std::string();
	...
	return StringFormat(("%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"),
...
void LexFromString(FMD5Hash& Hash, const char* Buffer)
{
	auto HexCharacterToDecimalValue = [](const char InHexChar, uint8_t& OutDecValue) -> bool
	{
		char Base = 0;
	...
	uint8_t Bytes[16];
	for (int32_t ByteIndex = 0, BufferIndex = 0; ByteIndex < 16; ++ByteIndex)
	{
		const char FirstChar = Buffer[BufferIndex++];
		if (FirstChar == '\0')
		{
			return;
		}

		const char SecondChar = Buffer[BufferIndex++];
		if (SecondChar == '\0')
		{
			return;
		}

		uint8_t FirstCharVal, SecondCharVal;
...
FMD5Hash FMD5Hash::HashFile(const char* InFilename, std::vector<uint8_t>* Buffer)
{
	//FArchive* Ar = IFileManager::Get().CreateFileReader(InFilename);
	//FMD5Hash Result = HashFileFromArchive( Ar, Buffer );
	//delete Ar;
	YFile asset_file(InFilename, YFile::FileType(YFile::FT_BINARY | YFile::FT_Read));
	std::unique_ptr<MemoryFile> mem_file = asset_file.ReadFile();
	FMD5 MD5;
	MD5.Update(mem_file->GetData(), mem_file->GetSize());
	FMD5Hash Hash;
	Hash.Set(MD5);
	return Hash;
}

//FMD5Hash FMD5Hash::HashFileFromArchive( FArchive* Ar, TArray<uint8_t>* Buffer)
//{
//	FMD5Hash Hash;
//	if (Ar)
//	{
//		TArray<uint8_t> LocalScratch;
//		if (!Buffer)
//		{
...
uint32_t GetTypeHash(FSHAHash const& InKey)
{
	assert(0);
	return 0;
...
void FSHA1::Transform(uint32_t *state, const uint8_t *buffer)
{
...
void FSHA1::Update(const uint8_t *data, uint32_t len)
{
	uint32_t i, j;
...
void FSHA1::UpdateWithString(const char *String, uint32_t Length)
{
	Update((const uint8_t*)String, Length );
}
...
void FSHA1::GetHash(uint8_t *puDest)
{
...
*/
void FSHA1::HashBuffer(const void* Data, uint32_t DataSize, uint8_t* OutHash)
{
	// do an atomic hash operation
	FSHA1 Sha;
	Sha.Update((const uint8_t*)Data, DataSize);
	Sha.Final();
	Sha.GetHash(OutHash);
}
...
void FSHA1::HMACBuffer(const void* Key, uint32_t KeySize, const void* Data, uint32_t DataSize, uint8_t* OutHash)
{
	const uint8_t BlockSize = 64;
	const uint8_t HashSize = 20;
	...
	uint8_t* IKeyPad_Data = new uint8_t[ARRAY_COUNT(IKeyPad) + DataSize];
	...
	uint8_t OKeyPad_IHash[ARRAY_COUNT(OKeyPad) + HashSize];
 ...
 */
void FSHA1::InitializeFileHashesFromBuffer(uint8_t* Buffer, int32_t BufferSize, bool bDuplicateKeyMemory)
{
	...
	while (Offset < BufferSize)
	{
		...
		{
			// skip over the file
			Offset +=(int32_t) strlen(Filename) + 1;
			...
			(bIsDoingFullFileHashes ? FullFileSHAHashMap : ScriptSHAHashMap)[std::string(Filename)] =  Hash;
 ...
 */
bool FSHA1::GetFileSHAHash(const char* Pathname, uint8_t Hash[20], bool bIsFullPackageHash)
{
	// look for this file in the hash
	//uint8_t** HashData = (bIsFullPackageHash ? FullFileSHAHashMap : ScriptSHAHashMap).Find(FPaths::GetCleanFilename(Pathname).ToLower());
	//uint8_t** HashData = (bIsFullPackageHash ? FullFileSHAHashMap : ScriptSHAHashMap).Find(FPaths::GetCleanFilename(Pathname).ToLower());
	
	auto find_result = (bIsFullPackageHash ? FullFileSHAHashMap : ScriptSHAHashMap).find(Pathname);
	if (find_result != (bIsFullPackageHash ? FullFileSHAHashMap : ScriptSHAHashMap).end())
	{
		// do we want a copy?
		if ( Hash)
		{
			// return it
			memcpy(Hash, find_result->second, 20);
		}
		return true;
	}
	return false;
...
void FAsyncSHAVerify::DoWork()
{
	...
	if (Pathname.size() > 0)
	{
		...
		if (FSHA1::GetFileSHAHash(Pathname.c_str(), Hash) == false)
		{
	...
	if (bFailed)
	{
		appOnFailSHAVerification(Pathname.c_str(), bFailedHashLookup);
	}
}
...
void appOnFailSHAVerification(const char* FailedPathname, bool bFailedDueToMissingHash)
{
	...
	//UE_LOG(LogSHA, Log, TEXT("SHA Verification failed for '%s'. Reason: %s"), 
		//FailedPathname ? FailedPathname : TEXT("Unknown file"),
		//bFailedDueToMissingHash ? TEXT("Missing hash") : TEXT("Bad hash"));
	ERROR_INFO("SHA Verification failed");
#endif
}
