...
FRenderResource::~FRenderResource()
{
	if (bInitialized && !GIsCriticalError)
	{
		...
		ERROR_INFO("A FRenderResource was deleted without being released first!");
	}
}
...
void ReleaseResourceAndFlush(FRenderResource* Resource)
{
	...
	//		Resource->ReleaseResource();
	//	});
	ENQUEUE_RENDER_COMMAND(ReleaseCommandAndFlush)(
		[Resource](FRHICommandListImmediate& RHICmdList)
		{
			Resource->ReleaseResource();
		});
...
std::string FTextureReference::GetFriendlyName() const
{
...
public:
	...
	explicit FDynamicVertexBuffer(uint32 InMinBufferSize)
		: MappedBuffer(NULL)
		, BufferSize(YMath::Max<uint32>(YMath::Align(InMinBufferSize,ALIGNMENT),ALIGNMENT))
...
struct FDynamicVertexBufferPool
{
	...
	std::vector<FDynamicVertexBuffer> VertexBuffers;
	...
	~FDynamicVertexBufferPool()
	{
		int32 NumVertexBuffers = VertexBuffers.size();
...
FGlobalDynamicVertexBuffer::FAllocation FGlobalDynamicVertexBuffer::Allocate(uint32 SizeInBytes)
{
	...
	if (VertexBuffer == NULL || VertexBuffer->AllocatedByteCount + SizeInBytes > VertexBuffer->BufferSize)
	{
		...
		VertexBuffer = NULL;
		for (int32 BufferIndex = 0, NumBuffers = Pool->VertexBuffers.size(); BufferIndex < NumBuffers; ++BufferIndex)
		{
		...
		if (VertexBuffer == NULL)
		{
			...
			Pool->VertexBuffers.push_back(FDynamicVertexBuffer(SizeInBytes));
			VertexBuffer = &Pool->VertexBuffers.back();
...
struct FDynamicIndexBufferPool
{
	...
	std::vector<FDynamicIndexBuffer> IndexBuffers;
	/** The current buffer from which allocations are being made. */
	...
	{
		int32 NumIndexBuffers = IndexBuffers.size();
...
FGlobalDynamicIndexBuffer::FAllocation FGlobalDynamicIndexBuffer::Allocate(uint32 NumIndices, uint32 IndexStride)
{
	...
	if (IndexBuffer == NULL || IndexBuffer->AllocatedByteCount + SizeInBytes > IndexBuffer->BufferSize)
	{
		// Find a buffer in the pool big enough to service the request.
		IndexBuffer = NULL;
		for (int32 BufferIndex = 0, NumBuffers = Pool->IndexBuffers.size(); BufferIndex < NumBuffers; ++BufferIndex)
		{
		...
		if (IndexBuffer == NULL)
		{
			...
			Pool->IndexBuffers.push_back(FDynamicIndexBuffer(SizeInBytes, Pool->BufferStride));
			IndexBuffer = &Pool->IndexBuffers.back();
...
void FMipBiasFade::SetNewMipCount( float ActualMipCount, float TargetMipCount, double LastRenderTime, EMipFadeSettings FadeSetting )
{
	...
	float TimeSinceLastRendered = float(YEngine::GetEngine()->GetCurrentGameTime() - LastRenderTime);
	...
	// Is there no change?
	if ( YMath::IsNearlyEqual(TotalMipCount, ActualMipCount) && YMath::IsNearlyEqual(TargetMipCount, CurrentTargetMipCount) )
	{
		return;
	}
	...
	CurrentInterpolatedMipCount = YMath::Clamp<float>(CurrentInterpolatedMipCount, 0, ActualMipCount);
	...
	if ( YMath::IsNearlyZero(MipCountDelta) )
	{
		MipCountDelta = 0.0f;
		BiasOffset = 0.0f;
		MipCountFadingRate = 0.0f;
