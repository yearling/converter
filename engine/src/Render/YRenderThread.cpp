...
//
 ...
 std::atomic<bool> GMainThreadBlockedOnRenderThread;
...
{
	while (GIsRenderingThreadSuspended.load(std::memory_order::memory_order_relaxed))
	{
...
FSuspendRenderingThread::FSuspendRenderingThread(bool bInRecreateThread)
{
	...
	{
		if (GIsRenderingThreadSuspended.load(std::memory_order::memory_order_relaxed) == 0)
		{
			...
			/*	FGraphEventRef CompleteHandle = FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
				...
				FGraphEventRef CompleteHandle = FFunctionGraphTask::CreateAndDispatchWhenReady([this]() { SuspendRendering(); }, nullptr, RenderThread);
				...
				check(GIsRenderingThreadSuspended.load(std::memory_order::memory_order_relaxed));

				// Now tell the render thread to busy wait until it's resumed
				DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.WaitAndResumeRendering"),STAT_FSimpleDelegateGraphTask_WaitAndResumeRendering,STATGROUP_TaskGraphTasks);
				...
				FFunctionGraphTask::CreateAndDispatchWhenReady([this]() { WaitAndResumeRendering(); }, nullptr, RenderThread);
			}
			else
			{
				SuspendRendering();
...
FSuspendRenderingThread::~FSuspendRenderingThread()
{
	if (bRecreateThread)
	{
		...
		if (bUseRenderingThread && bWasRenderingThreadRunning)
		{
			...
			FFunctionGraphTask::CreateAndDispatchWhenReady([this]() { FPlatformProcess::SetRealTimeMode(); }, nullptr, ENamedThreads::GetRenderThread());
 ...
 */
std::atomic<bool> GRunRenderingThreadHeartbeat;
...
public:
	...
	{
		while (GRunRenderingThreadHeartbeat.load(std::memory_order::memory_order_relaxed))
		{
			FPlatformProcess::Sleep(1.f / (4.0f * GRenderingThreadMaxIdleTickFrequency));
			if (!GIsRenderingThreadSuspended.load(std::memory_order::memory_order_relaxed) && OutstandingHeartbeats.GetValue() < 4)
			{
				...
				//if (!GIsRenderingThreadSuspended.load(std::memory_order::memory_order_relaxed))
				//{
...
//	}
//
//	virtual void OnCVarChange(std::string& Dest, const std::string& NewValue)
//	{
//		ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
//			OnCVarChange3,
...
static std::string BuildRenderingThreadName(uint32 ThreadIndex)
{
	//return std::string::Printf(TEXT("%s %u"), *FName(NAME_RenderThread).GetPlainNameString(), ThreadIndex);
	return StringFormat("Render Thread %u", ThreadIndex);
}
...
public:
	...
	**/
	...
	//{
	//}
	FOwnershipOfRHIThreadTask(bool bInAcquireOwnership)
		: bAcquireOwnership(bInAcquireOwnership)
	{
	}
	/**
...
void StartRenderingThread()
{
	...
	std::string thread_name = StringFormat("RTHeartBeat %d", ThreadCount);
	GRenderingThreadHeartbeat = FRunnableThread::Create(GRenderingThreadRunnableHeartbeat, thread_name, 16 * 1024, TPri_AboveNormal, FPlatformAffinity::GetRTHeartBeatMask());
...
void StartRenderCommandFenceBundler()
{
	...
	Prereqs.push_back(BundledCompletionEventPrereq);
...
struct FRHISyncFrameCommand final : public FRHICommand<FRHISyncFrameCommand>
{
	...
	void Execute(FRHICommandListBase& CmdList)
	{
		if (GTSyncType == 1)
		{
			...
			std::vector<FBaseGraphTask*> Subsequents;
...
void FRenderCommandFence::BeginFence(bool bSyncToRHIAndGPU)
{
	...
	else
	{
		...
		if (bSyncToRHIAndGPU)
		{
			...
			ENQUEUE_RENDER_COMMAND(FSyncFrameCommand)([GTSyncType, this](FRHICommandListImmediate& RHICmdList) {
				if (GRHIThread_InternalUseOnly)
				{
					new (RHICmdList.AllocCommand<FRHISyncFrameCommand>()) FRHISyncFrameCommand(CompletionEvent, GTSyncType);
					RHICmdList.ImmediateFlush(EImmediateFlushType::DispatchToRHIThread);
...
void FlushPendingDeleteRHIResources_GameThread()
{
	if (!IsRunningRHIInSeparateThread())
	{
		...
		ENQUEUE_RENDER_COMMAND(FlushPendingDeleteRHIResources)([](FRHICommandList&) {
			FlushPendingDeleteRHIResources_RenderThread();
			});
	}
}
...
std::vector<FDeferredCleanupInterface*> PendingCleanupObjectsList;
...
FPendingCleanupObjects::FPendingCleanupObjects()
{
	check(IsInGameThread());
	{
		...
		std::swap(CleanupArray, PendingCleanupObjectsList);
	}
}
...
void BeginCleanup(FDeferredCleanupInterface* CleanupObject)
{
	{
		FScopeLock Lock(&PendingCleanupObjectsListLock);
		PendingCleanupObjectsList.push_back(CleanupObject);
	}
}
...
void SetRHIThreadEnabled(bool bEnableDedicatedThread, bool bEnableRHIOnTaskThreads)
{
	if (bEnableDedicatedThread != GUseRHIThread_InternalUseOnly || bEnableRHIOnTaskThreads != GUseRHITaskThreads_InternalUseOnly)
	{
		if ((bEnableRHIOnTaskThreads || bEnableDedicatedThread) && !GIsThreadedRendering)
		{
			...
			WARNING_INFO("Can't switch to RHI thread mode when we are not running a multithreaded renderer.");
		}
		else
		{
			StopRenderingThread();
	...
	if (IsRunningRHIInSeparateThread())
	{
		if (IsRunningRHIInDedicatedThread())
		{
			...
			WARNING_INFO("RHIThread is now running on a dedicated thread.");
		}
		else
		{
			check(IsRunningRHIInTaskThread());
			WARNING_INFO("RHIThread is now running on task threads.");
		}
	}
	else
	{
		check(!IsRunningRHIInTaskThread() && !IsRunningRHIInDedicatedThread());
		WARNING_INFO("RHIThread is disabled.");
	}

}

static void HandleRHIThreadEnableChanged(const std::vector<std::string>& Args)
{
	if (Args.size() > 0)
	{
		const int32 UseRHIThread = atoi(Args[0].c_str());
