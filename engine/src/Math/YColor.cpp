...
FLinearColor::FLinearColor(const YVector& Vector) :
	R(Vector.x),
	G(Vector.y),
	B(Vector.z),
	A(1.0f)
...
FLinearColor::FLinearColor(const YVector4& Vector) :
	R(Vector.x),
	G(Vector.y),
	B(Vector.z),
	A(Vector.w)
...
{
	const float	Primary = YMath::Max(R, YMath::Max(G, B));
	FColor	Color;
	...
	else
	{
		...
		Color.A = YMath::Clamp(YMath::TruncToInt((float)Exponent), -128, 127) + 128;
	}
...
{
	return FColor(
		(uint8_t)YMath::Clamp<int32_t>(YMath::TruncToInt(R * 255.f), 0, 255),
		(uint8_t)YMath::Clamp<int32_t>(YMath::TruncToInt(G * 255.f), 0, 255),
		(uint8_t)YMath::Clamp<int32_t>(YMath::TruncToInt(B * 255.f), 0, 255),
		(uint8_t)YMath::Clamp<int32_t>(YMath::TruncToInt(A * 255.f), 0, 255)
	);
}
...
{
	return FColor(
		(uint8_t)YMath::Clamp<int32_t>(YMath::RoundToInt(R * 255.f), 0, 255),
		(uint8_t)YMath::Clamp<int32_t>(YMath::RoundToInt(G * 255.f), 0, 255),
		(uint8_t)YMath::Clamp<int32_t>(YMath::RoundToInt(B * 255.f), 0, 255),
		(uint8_t)YMath::Clamp<int32_t>(YMath::RoundToInt(A * 255.f), 0, 255)
	);
}
...
FColor FColor::FromHex(const std::string& HexString)
{
	...
	//int32_t StartIndex = (!HexString.IsEmpty() && HexString[0] == TCHAR('#')) ? 1 : 0;

	//if (HexString.Len() == 3 + StartIndex)
	//{
	//	const int32_t R = FParse::HexDigit(HexString[StartIndex++]);
	//	const int32_t G = FParse::HexDigit(HexString[StartIndex++]);
	//	const int32_t B = FParse::HexDigit(HexString[StartIndex]);
...
FLinearColor FLinearColor::FGetHSV(uint8_t H, uint8_t S, uint8_t V)
{
	...
	const YVector Hue = (H < 86) ? YVector((85 - H) / 85.f, (H - 0) / 85.f, 0) : (H < 171) ? YVector(0, (170 - H) / 85.f, (H - 85) / 85.f) : YVector((H - 170) / 85.f, 0, (255 - H) / 84.f);
	const YVector ColorVector = (Hue + S / 255.f * (YVector(1, 1, 1) - Hue)) * Brightness;
...
{
	const float RGBMin = YMath::Min(R, YMath::Min(G, B));
	const float RGBMax = YMath::Max(R, YMath::Max(G, B));
...
{
	const uint8_t Hue = (uint8_t)(YMath::FRand() * 255.f);
...
FColor FColor::MakeRedToGreenColorFromScalar(float Scalar)
{
	...
	int32_t R = YMath::TruncToInt(255 * RedSclr);
	int32_t G = YMath::TruncToInt(255 * GreenSclr);
	int32_t B = 0;
	return FColor(R, G, B);
}
