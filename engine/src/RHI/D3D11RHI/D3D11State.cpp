...
static std::unordered_map<ID3D11SamplerState*, FD3D11SamplerState*> GSamplerStateCache;
...
FSamplerStateRHIRef FD3D11DynamicRHI::RHICreateSamplerState(const FSamplerStateInitializerRHI& Initializer)
{
	...
	VERIFYD3D11RESULT_EX(d3d_device_->CreateSamplerState(&SamplerDesc, SamplerStateHandle.GetInitReference()), d3d_device_);
	...
	auto find_result = GSamplerStateCache.find(SamplerStateHandle);
	if (find_result != GSamplerStateCache.end()) 
	{
...
FRasterizerStateRHIRef FD3D11DynamicRHI::RHICreateRasterizerState(const FRasterizerStateInitializerRHI& Initializer)
{
	...
	RasterizerDesc.DepthBias = YMath::FloorToInt(Initializer.DepthBias * (float)(1 << 24));
	...
	VERIFYD3D11RESULT_EX(d3d_device_->CreateRasterizerState(&RasterizerDesc,RasterizerState->Resource.GetInitReference()), d3d_device_);
...
FDepthStencilStateRHIRef FD3D11DynamicRHI::RHICreateDepthStencilState(const FDepthStencilStateInitializerRHI& Initializer)
{
	...
	VERIFYD3D11RESULT_EX(d3d_device_->CreateDepthStencilState(&DepthStencilDesc,DepthStencilState->Resource.GetInitReference()), d3d_device_);
...
FBlendStateRHIRef FD3D11DynamicRHI::RHICreateBlendState(const FBlendStateInitializerRHI& Initializer)
{
	...
	VERIFYD3D11RESULT_EX(d3d_device_->CreateBlendState(&BlendDesc,BlendState->Resource.GetInitReference()), d3d_device_);
	return BlendState;
}
