	...
	for (int32 BucketIndex = 0; BucketIndex < NumPoolBuckets; BucketIndex++)
	{
		for (int32 EntryIndex = UniformBufferPool[BucketIndex].size() - 1; EntryIndex >= 0 && NumCleaned < 10; EntryIndex--)
		{
			...
			if (GFrameNumberRenderThread - PoolEntry.FrameFreed > 30)
			{
				...
				UniformBufferPool[BucketIndex].erase(UniformBufferPool[BucketIndex].begin()+EntryIndex);
			}
		}
	}
...
FUniformBufferRHIRef FD3D11DynamicRHI::RHICreateUniformBuffer(const void* Contents,const FRHIUniformBufferLayout& Layout,EUniformBufferUsage Usage)
{
	...
	if (NumBytes > 0)
	{
		...
		check(YMath::Align(NumBytes,16) == NumBytes);
		check(YMath::Align(Contents,16) == Contents);
		...
		if (IsPoolingEnabled())
		{
			...
			if (!RingAllocation.IsValid())
			{
				...
				if (PoolBucket.size() > 0)
				{
					...
					FPooledUniformBuffer FreeBufferEntry = PoolBucket.back();
					PoolBucket.erase(PoolBucket.end() - 1);
				...
				if (!IsValidRef(UniformBufferResource))
				{
					...
					VERIFYD3D11RESULT_EX(d3d_device_->CreateBuffer(&Desc, NULL, UniformBufferResource.GetInitReference()), d3d_device_);
				...
				VERIFYD3D11RESULT_EX(d3d_dc_->Map(UniformBufferResource, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedSubresource), d3d_device_);
				check(MappedSubresource.RowPitch >= NumBytes);
				memcpy(MappedSubresource.pData, Contents, NumBytes);
				d3d_dc_->Unmap(UniformBufferResource, 0);
			}
		...
		else
		{
			...
			TRefCountPtr<ID3D11Buffer> UniformBufferResource;
			VERIFYD3D11RESULT_EX(d3d_device_->CreateBuffer(&Desc,&ImmutableData,UniformBufferResource.GetInitReference()), d3d_device_);
	...
	if (Layout.Resources.size())
	{
		...
		NewUniformBuffer->ResourceTable.clear();
		NewUniformBuffer->ResourceTable.reserve(NumResources);
		NewUniformBuffer->ResourceTable.assign(NumResources,nullptr);

		check(Layout.ResourceOffsets.size() == NumResources);
