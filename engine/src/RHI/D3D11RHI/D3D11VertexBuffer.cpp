...
FVertexBufferRHIRef FD3D11DynamicRHI::RHICreateVertexBuffer(uint32 Size,uint32 InUsage, FRHIResourceCreateInfo& CreateInfo)
{
	...
	/*	checkf(Size == CreateInfo.ResourceArray->GetResourceDataSize(),
		...
		assert(Size == CreateInfo.ResourceArray->GetResourceDataSize());
		if (Size != CreateInfo.ResourceArray->GetResourceDataSize())
		{
			WARNING_INFO(StringFormat("DebugName: %s, GPU Size: %d, CPU Size: %d, Is Dynamic: %s", CreateInfo.DebugName, Size, CreateInfo.ResourceArray->GetResourceDataSize(),
				InUsage & BUF_AnyDynamic ? "yes":"no"));
		}
#endif
	...
	HRESULT Result = d3d_device_->CreateBuffer(&Desc, pInitData, VertexBufferResource.GetInitReference());
	if (FAILED(Result))
	{
		...
		ERROR_INFO(StringFormat("D3DDevice failed CreateBuffer VB with ByteWidth=%d, BindFlags=0x%x Usage=%d, CPUAccess=0x%x, MiscFlags=0x%x", Desc.ByteWidth, (uint32)Desc.BindFlags, (uint32)Desc.Usage, Desc.CPUAccessFlags, Desc.MiscFlags));
...
void* FD3D11DynamicRHI::RHILockVertexBuffer(FVertexBufferRHIParamRef VertexBufferRHI,uint32 Offset,uint32 Size,EResourceLockMode LockMode)
{
	...
	if(bIsDynamic)
	{
		...
		VERIFYD3D11RESULT_EX(d3d_dc_->Map(VertexBuffer->Resource,0,D3D11_MAP_WRITE_DISCARD,0,&MappedSubresource), d3d_device_);
	...
	else
	{
		if(LockMode == RLM_ReadOnly)
		{
			...
			VERIFYD3D11RESULT_EX(d3d_device_->CreateBuffer(&StagingBufferDesc,NULL,StagingVertexBuffer.GetInitReference()), d3d_device_);
			...
			d3d_dc_->CopySubresourceRegion(StagingVertexBuffer,0,0,0,0,VertexBuffer->Resource,0,&SourceBox);

			// Map the staging buffer's memory for reading.
			D3D11_MAPPED_SUBRESOURCE MappedSubresource;
			VERIFYD3D11RESULT_EX(d3d_dc_->Map(StagingVertexBuffer,0,D3D11_MAP_READ,0,&MappedSubresource), d3d_device_);
			LockedData.SetData(MappedSubresource.pData);
			LockedData.Pitch = MappedSubresource.RowPitch;
		}
		else
	...
	OutstandingLocks[LockedKey] = LockedData;
...
void FD3D11DynamicRHI::RHIUnlockVertexBuffer(FVertexBufferRHIParamRef VertexBufferRHI)
{
	...
	FD3D11LockedData* LockedData = &OutstandingLocks[LockedKey];
	check(LockedData);

	if(bIsDynamic)
	{
		// If the VB is dynamic, its memory was mapped directly; unmap it.
		d3d_dc_->Unmap(VertexBuffer->Resource,0);
	}
	else
	{
		...
		if(LockedData->StagingResource)
		{
			...
			d3d_dc_->Unmap(StagingBuffer,0);
		}
		else 
		{
			// Copy the contents of the temporary memory buffer allocated for writing into the VB.
			d3d_dc_->UpdateSubresource(VertexBuffer->Resource,LockedKey.Subresource,NULL,LockedData->GetData(),LockedData->Pitch,0);
...
void FD3D11DynamicRHI::RHICopyVertexBuffer(FVertexBufferRHIParamRef SourceBufferRHI,FVertexBufferRHIParamRef DestBufferRHI)
{
	...
	d3d_dc_->CopyResource(DestBuffer->Resource,SourceBuffer->Resource);
