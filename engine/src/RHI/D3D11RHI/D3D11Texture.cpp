...
void UpdateD3D11TextureStats(uint32 BindFlags, uint32 MiscFlags, int64 TextureSize, bool b3D)
{
	...
	int64 AlignedSize = (TextureSize > 0) ? YMath::Align(TextureSize, 1024) / 1024 : -(YMath::Align(-TextureSize, 1024) / 1024);
	if(ShouldCountAsTextureMemory(BindFlags))
	{
...
struct FTexturePool
{
	std::vector<FPooledTexture2D> Textures;
};
...
void ReleasePooledTextures()
{
	for (int32 MipPoolIndex = 0; MipPoolIndex < FGlobalTexturePool::MipPoolCount; ++MipPoolIndex)
	{
		...
		{
			GTexturePool.Pools[MipPoolIndex][FormatPoolIndex].Textures.clear();
		}
	}
}
...
#define GUARDED_TEXTURE_CREATES 0
...
void SafeCreateTexture2D(ID3D11Device* Direct3DDevice, const D3D11_TEXTURE2D_DESC* TextureDesc, const D3D11_SUBRESOURCE_DATA* SubResourceData, ID3D11Texture2D** OutTexture2D)
{
	...
	__finally
	{
		...
		/*	UE_LOG(LogD3D11RHI,Error,
			...
			ERROR_INFO(StringFormat("Driver crashed while creating texture: %ux%ux%u %s(0x%08x) with %u mips", TextureDesc->Width,
				TextureDesc->Height,
				TextureDesc->ArraySize,
				GetD3D11TextureFormatString(TextureDesc->Format),
				(uint32)TextureDesc->Format,
	...
	uint32 NumMips,uint32 NumSamples,uint32 Flags, FRHIResourceCreateInfo& CreateInfo)
{
	...
	if (!IsValidRef(TextureResource))
	{
		...
		if (CreateInfo.BulkData)
		{
			...
			D3D11_SUBRESOURCE_DATA empty_tmp;
			ZeroMemory(&empty_tmp, sizeof(D3D11_SUBRESOURCE_DATA));
			SubResourceData.insert(SubResourceData.end(), NumMips * SizeZ, empty_tmp);
			...
			{			
				...
				for(uint32 MipIndex = 0;MipIndex < NumMips;++MipIndex)
				{
					...
					uint32 NumBlocksX = YMath::Max<uint32>(1,(SizeX >> MipIndex) / GPixelFormats[Format].BlockSizeX);
					uint32 NumBlocksY = YMath::Max<uint32>(1,(SizeY >> MipIndex) / GPixelFormats[Format].BlockSizeY);
...
#endif
		{
			SafeCreateTexture2D(d3d_device_, &TextureDesc, CreateInfo.BulkData != NULL ? (const D3D11_SUBRESOURCE_DATA*)SubResourceData.data() : NULL, TextureResource.GetInitReference());
		}
			...
			for (uint32 MipIndex = 0; MipIndex < NumMips; MipIndex++)
			{
				if ((Flags & TexCreate_TargetArraySlicesIndependently) && (bTextureArray || bCubeTexture))
				{
					...
					for (uint32 SliceIndex = 0; SliceIndex < TextureDesc.ArraySize; SliceIndex++)
					{
						...
						VERIFYD3D11RESULT_EX(d3d_device_->CreateRenderTargetView(TextureResource,&RTVDesc,RenderTargetView.GetInitReference()), d3d_device_);
		...
		if(bCreateDSV)
		{
			...
			for (uint32 AccessType = 0; AccessType < FExclusiveDepthStencil::MaxIndex; ++AccessType)
			{
				...
				if(d3d_device_->GetFeatureLevel() == D3D_FEATURE_LEVEL_11_0 || d3d_device_->GetFeatureLevel() == D3D_FEATURE_LEVEL_11_1)
				{
				...
				VERIFYD3D11RESULT_EX(d3d_device_->CreateDepthStencilView(TextureResource,&DSVDesc,DepthStencilViews[AccessType].GetInitReference()), d3d_device_);
			}
		}
	}
	...
	{
		{
			...
			VERIFYD3D11RESULT_EX(d3d_device_->CreateShaderResourceView(TextureResource,&SRVDesc,ShaderResourceView.GetInitReference()), d3d_device_);
		}
...
FD3D11Texture3D* FD3D11DynamicRHI::CreateD3D11Texture3D(uint32 SizeX,uint32 SizeY,uint32 SizeZ,uint8 Format,uint32 NumMips,uint32 Flags,FRHIResourceCreateInfo& CreateInfo)
{
	...
	if (CreateInfo.BulkData)
	{
		...
		D3D11_SUBRESOURCE_DATA tmp_data;
		ZeroMemory(&tmp_data, sizeof(D3D11_SUBRESOURCE_DATA));
		SubResourceData.resize(NumMips, tmp_data);
		uint32 MipOffset = 0;
		for(uint32 MipIndex = 0;MipIndex < NumMips;++MipIndex)
		{
			SubResourceData[MipIndex].pSysMem = &Data[MipOffset];
			SubResourceData[MipIndex].SysMemPitch      =  YMath::Max<uint32>(1,SizeX >> MipIndex) * GPixelFormats[Format].BlockBytes;
			SubResourceData[MipIndex].SysMemSlicePitch =  YMath::Max<uint32>(1,SizeY >> MipIndex) * SubResourceData[MipIndex].SysMemPitch;
			MipOffset                                  += YMath::Max<uint32>(1,SizeZ >> MipIndex) * SubResourceData[MipIndex].SysMemSlicePitch;
		}
	}
	...
	VERIFYD3D11CREATETEXTURERESULT(
		d3d_device_->CreateTexture3D(&TextureDesc,CreateInfo.BulkData != NULL ? (const D3D11_SUBRESOURCE_DATA*)SubResourceData.data() : NULL,TextureResource.GetInitReference()),
		SizeX,
		SizeY,
		SizeZ,
...
FTexture2DRHIRef FD3D11DynamicRHI::RHIAsyncCreateTexture2D(uint32 SizeX,uint32 SizeY,uint8 Format,uint32 NumMips,uint32 Flags,void** InitialMipData,uint32 NumInitialMips)
{
	...
	ZeroMemory( SubResourceData, sizeof( D3D11_SUBRESOURCE_DATA ) * MAX_TEXTURE_MIP_COUNT );
	...
	for (uint32 MipIndex = NumInitialMips; MipIndex < NumMips; ++MipIndex)
	{
		...
		if (MipSize > TempBufferSize)
		{
			...
			WARNING_INFO(StringFormat("Temp texture streaming buffer not large enough, needed %d bytes",MipSize));
			check(TempBufferSize == ZeroBufferSize);
			TempBufferSize = MipSize;
			TempBuffer = malloc(TempBufferSize);
			ZeroMemory(TempBuffer,TempBufferSize);
		}
	...
	SafeCreateTexture2D(d3d_device_,&TextureDesc,SubResourceData,TextureResource.GetInitReference());
...
void FD3D11DynamicRHI::RHICopySharedMips(FTexture2DRHIParamRef DestTexture2DRHI,FTexture2DRHIParamRef SrcTexture2DRHI)
{
	...
	const uint32 NumSharedMips = YMath::Min(DestTexture2D->GetNumMips(),SrcTexture2D->GetNumMips());
...
FShaderResourceViewRHIRef FD3D11DynamicRHI::RHICreateShaderResourceView(FTexture2DRHIParamRef Texture2DRHI, uint8 MipLevel)
{
	...
	SRVDesc.Format = PlatformShaderResourceFormat;
	TRefCountPtr<ID3D11ShaderResourceView> ShaderResourceView;
	VERIFYD3D11RESULT_EX(d3d_device_->CreateShaderResourceView(Texture2D->GetResource(), &SRVDesc, (ID3D11ShaderResourceView**)ShaderResourceView.GetInitReference()), d3d_device_);
...
FShaderResourceViewRHIRef FD3D11DynamicRHI::RHICreateShaderResourceView(FTexture2DRHIParamRef Texture2DRHI, uint8 MipLevel, uint8 NumMipLevels, uint8 Format)
{
	...
	HRESULT hResCreateShaderResourceView = d3d_device_->CreateShaderResourceView(Texture2D->GetResource(), &SRVDesc, (ID3D11ShaderResourceView**)ShaderResourceView.GetInitReference());
	...
	if(FAILED(hResCreateShaderResourceView))
	{
		...
		WARNING_INFO(StringFormat("CreateShaderResourceView failed, input: ViewDim:%d MSAA:%d Format:%d/%d SRGB:%d hRes:%x", (int32)SRVDesc.ViewDimension, (int32)TextureDesc.SampleDesc.Count, (int32)PlatformResourceFormat, (int32)PlatformShaderResourceFormat, bSRGB ? 1 : 0, hResCreateShaderResourceView));
	}

	VERIFYD3D11RESULT_EX(hResCreateShaderResourceView, d3d_device_);
...
FShaderResourceViewRHIRef FD3D11DynamicRHI::RHICreateShaderResourceView(FTexture3DRHIParamRef Texture3DRHI, uint8 MipLevel)
{
	...
	SRVDesc.Format = PlatformShaderResourceFormat;
	TRefCountPtr<ID3D11ShaderResourceView> ShaderResourceView;
	VERIFYD3D11RESULT_EX(d3d_device_->CreateShaderResourceView(Texture3D->GetResource(), &SRVDesc, (ID3D11ShaderResourceView**)ShaderResourceView.GetInitReference()), d3d_device_);
...
FShaderResourceViewRHIRef FD3D11DynamicRHI::RHICreateShaderResourceView(FTexture2DArrayRHIParamRef Texture2DArrayRHI, uint8 MipLevel)
{
	...
	SRVDesc.Format = PlatformShaderResourceFormat;
	TRefCountPtr<ID3D11ShaderResourceView> ShaderResourceView;
	VERIFYD3D11RESULT_EX(d3d_device_->CreateShaderResourceView(Texture2DArray->GetResource(), &SRVDesc, (ID3D11ShaderResourceView**)ShaderResourceView.GetInitReference()), d3d_device_);
...
FShaderResourceViewRHIRef FD3D11DynamicRHI::RHICreateShaderResourceView(FTextureCubeRHIParamRef TextureCubeRHI, uint8 MipLevel)
{
	...
	SRVDesc.Format = PlatformShaderResourceFormat;
	TRefCountPtr<ID3D11ShaderResourceView> ShaderResourceView;
	VERIFYD3D11RESULT_EX(d3d_device_->CreateShaderResourceView(TextureCube->GetResource(), &SRVDesc, (ID3D11ShaderResourceView**)ShaderResourceView.GetInitReference()), d3d_device_);
...
void FD3D11DynamicRHI::RHIGenerateMips(FTextureRHIParamRef TextureRHI)
{
	...
	d3d_dc_->GenerateMips(Texture->GetShaderResourceView());
...
FTexture2DRHIRef FD3D11DynamicRHI::RHIAsyncReallocateTexture2D(FTexture2DRHIParamRef Texture2DRHI, int32 NewMipCount, int32 NewSizeX, int32 NewSizeY, FThreadSafeCounter* RequestStatus)
{
	...
	// Use the GPU to asynchronously copy the old mip-maps into the new texture.
	const uint32 NumSharedMips = YMath::Min(Texture2D->GetNumMips(),NewTexture2D->GetNumMips());
...
void* TD3D11Texture2D<RHIResourceType>::Lock(uint32 MipIndex,uint32 ArrayIndex,EResourceLockMode LockMode,uint32& DestStride)
{
	...
	const uint32 MipSizeX = YMath::Max(this->GetSizeX() >> MipIndex,BlockSizeX);
	const uint32 MipSizeY = YMath::Max(this->GetSizeY() >> MipIndex,BlockSizeY);
	...
	D3DRHI->OutstandingLocks[FD3D11LockedKey(GetResource(),Subresource)] = LockedData;
...
void TD3D11Texture2D<RHIResourceType>::Unlock(uint32 MipIndex,uint32 ArrayIndex)
{
	...
	//check(LockedData);
	assert(D3DRHI->OutstandingLocks.count(LockedKey));
	FD3D11LockedData* LockedData = &D3DRHI->OutstandingLocks[LockedKey];
...
void FD3D11DynamicRHI::RHIUpdateTexture2D(FTexture2DRHIParamRef TextureRHI,uint32 MipIndex,const FUpdateTextureRegion2D& UpdateRegion,uint32 SourcePitch,const uint8* SourceData)
{
	...
	d3d_dc_->UpdateSubresource(Texture->GetResource(), MipIndex, &DestBox, SourceData, SourcePitch, 0);
}
...
void FD3D11DynamicRHI::RHIUpdateTexture3D(FTexture3DRHIParamRef TextureRHI,uint32 MipIndex,const FUpdateTextureRegion3D& UpdateRegion,uint32 SourceRowPitch,uint32 SourceDepthPitch,const uint8* SourceData)
{
	...
	const int32 NumBlockX = YMath::DivideAndRoundUp<int32>(UpdateRegion.Width, Format.BlockSizeX);
	const int32 NumBlockY = YMath::DivideAndRoundUp<int32>(UpdateRegion.Height, Format.BlockSizeY);
	...
	d3d_dc_->UpdateSubresource(Texture->GetResource(), MipIndex, &DestBox, SourceData, SourceRowPitch, SourceDepthPitch);
}
...
void FD3D11DynamicRHI::RHIBindDebugLabelName(FTextureRHIParamRef TextureRHI, const char* Name)
{
	...
	{
		Texture2D->GetResource()->SetPrivateData(WKPDID_D3DDebugObjectName, strlen(Name) + 1, Name);
	}
	else if (FD3D11TextureCube* TextureCube = (FD3D11TextureCube*)TextureRHI->GetTextureCube())
	{
		TextureCube->GetResource()->SetPrivateData(WKPDID_D3DDebugObjectName, strlen(Name) + 1, Name);
	}
	else if (FD3D11Texture3D* Texture3D = (FD3D11Texture3D*)TextureRHI->GetTexture3D())
	{
		Texture3D->GetResource()->SetPrivateData(WKPDID_D3DDebugObjectName, strlen(Name) + 1, Name);
	}
#endif
}
...
void FD3D11DynamicRHI::RHICopySubTextureRegion(FTexture2DRHIParamRef SourceTextureRHI, FTexture2DRHIParamRef DestinationTextureRHI, FBox2D SourceBox, FBox2D DestinationBox)
{
	...
	//	return;
	//}
	if (!(bValidSrc && bValidDest))
	{
		WARNING_INFO(StringFormat("Invalid copy detected for RHICopySubTextureRegion. Skipping copy.  SrcBox: left:%i, right:%i, top:%i, bottom:%i, DstBox:left:%i, right:%i, top:%i, bottom:%i,  SrcTexSize: %i x %i, DestTexSize: %i x %i ",
			SourceBox.Min.x,
				SourceBox.Max.x,
				SourceBox.Min.y,
				SourceBox.Max.y,
	...
	d3d_dc_->CopySubresourceRegion(DestinationRessource, 0, DestinationBox.Min.x + DestinationOffsetX, DestinationBox.Min.y + DestinationOffsetY, 0, SourceTexture->GetResource(), 0, &SourceBoxAdjust);
}
