...
void FD3D11StateCacheBase::VerifySamplerStates()
{
	...
	//	checkf(SamplerStates[Index] == CurrentSamplerStates[ShaderFrequency][Index], TEXT("Dangling bound SamplerState, try running with -d3debug to track it down."));
		if (SamplerStates[Index] != CurrentSamplerStates[ShaderFrequency][Index])
		{
			ERROR_INFO("Dangling bound SamplderState");
		}
...
void FD3D11StateCacheBase::VerifyConstantBuffers()
{
	...
	for (uint32 Index = 0; Index < D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT; Index++)
	{
		...
		if (Buffers[Index] != CurrentConstantBuffers[ShaderFrequency][Index].Buffer)
		{
			ERROR_INFO("Dangling bound Constant Buffer");
		}
		if (Buffers[Index])
		{
...
void FD3D11StateCacheBase::VerifyShaderResourceViews()
{
	...
	for (uint32 Index = 0; Index < D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT; Index++)
	{
		...
		if (Views[Index] != CurrentShaderResourceViews[ShaderFrequency][Index])
		{
			ERROR_INFO("Dangling bound SRV");
		}
		if (Views[Index])
		{
			Views[Index]->Release();
...
void FD3D11StateCacheBase::VerifyCacheState()
{
	...
	// Verify Blend State
	{
		...
		check(memcmp(BlendFactor, CurrentBlendFactor, sizeof(CurrentBlendFactor)) == 0);
	...
	// Verify Viewport state
	{
		...
		check(memcmp(&vp, &CurrentViewport[0], sizeof(D3D11_VIEWPORT) * CurrentNumberOfViewports) == 0);
	}
...
void FD3D11StateCacheBase::ClearState()
{
	...
	memset(CurrentSamplerStates, 0, sizeof(CurrentSamplerStates));
