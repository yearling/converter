...
struct FD3D11VertexDeclarationKey
{
	...
	explicit FD3D11VertexDeclarationKey(const FVertexDeclarationElementList& InElements)
	{
		...
		memset(&StreamStrides, 0, sizeof(uint16)* MaxVertexElementCount);
		...
		for(int32 ElementIndex = 0;ElementIndex < InElements.size();ElementIndex++)
		{
			...
			default: ERROR_INFO(StringFormat("Unknown RHI vertex element type %u", (uint8)InElements[ElementIndex].Type));
			};
...
bool operator == (const D3D11_INPUT_ELEMENT_DESC& A, const D3D11_INPUT_ELEMENT_DESC& B)
{
	return strcmp(A.SemanticName, B.SemanticName) == 0 && A.SemanticIndex == B.SemanticIndex && A.Format == B.Format && A.AlignedByteOffset == B.AlignedByteOffset && A.InputSlotClass == B.InputSlotClass && A.InstanceDataStepRate == B.InstanceDataStepRate;
}
/** Compare two vertex declaration keys. */
bool operator==(const FD3D11VertexDeclarationKey& A, const FD3D11VertexDeclarationKey& B)
{
	return std::equal(A.VertexElements.begin(), A.VertexElements.end(),B.VertexElements.begin())
		&& !memcmp(A.StreamStrides, B.StreamStrides, sizeof(A.StreamStrides));
}
...
namespace std
{
	...
	template <>
	struct hash<FD3D11VertexDeclarationKey>
	{
		size_t operator()(const FD3D11VertexDeclarationKey& k) const
		{
...
std::unordered_map<FD3D11VertexDeclarationKey,FVertexDeclarationRHIRef> GVertexDeclarationCache;
...
FVertexDeclarationRHIRef FD3D11DynamicRHI::RHICreateVertexDeclaration(const FVertexDeclarationElementList& Elements)
{
	...
	FVertexDeclarationRHIRef VertexDeclarationRef;
	auto find_result = GVertexDeclarationCache.find(Key);
	if(find_result == GVertexDeclarationCache.end())
	{
		...
		GVertexDeclarationCache[Key] = new FD3D11VertexDeclaration(Key.VertexElements, Key.StreamStrides);
		VertexDeclarationRef = GVertexDeclarationCache[Key];
	}

	// The cached declaration must match the input declaration!
	check(VertexDeclarationRef);
	check(IsValidRef(VertexDeclarationRef));
	FD3D11VertexDeclaration* D3D11VertexDeclaration = (FD3D11VertexDeclaration*)VertexDeclarationRef.GetReference();
	checkSlow(D3D11VertexDeclaration->VertexElements == Key.VertexElements);

	return VertexDeclarationRef;
}
