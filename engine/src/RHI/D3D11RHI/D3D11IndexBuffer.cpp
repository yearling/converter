...
FIndexBufferRHIRef FD3D11DynamicRHI::RHICreateIndexBuffer(uint32 Stride,uint32 Size,uint32 InUsage, FRHIResourceCreateInfo& CreateInfo)
{
	...
	VERIFYD3D11RESULT_EX(d3d_device_->CreateBuffer(&Desc,pInitData,IndexBufferResource.GetInitReference()), d3d_device_);
...
void* FD3D11DynamicRHI::RHILockIndexBuffer(FIndexBufferRHIParamRef IndexBufferRHI,uint32 Offset,uint32 Size,EResourceLockMode LockMode)
{
	...
	if(bIsDynamic)
	{
		...
		VERIFYD3D11RESULT_EX(d3d_dc_->Map(IndexBuffer->Resource,0,D3D11_MAP_WRITE_DISCARD,0,&MappedSubresource), d3d_device_);
	...
	else
	{
		if(LockMode == RLM_ReadOnly)
		{
			...
			VERIFYD3D11RESULT_EX(d3d_device_->CreateBuffer(&StagingBufferDesc,NULL,StagingIndexBuffer.GetInitReference()), d3d_device_);
			LockedData.StagingResource = StagingIndexBuffer;

			// Copy the contents of the index buffer to the staging buffer.
			d3d_dc_->CopyResource(StagingIndexBuffer,IndexBuffer->Resource);

			// Map the staging buffer's memory for reading.
			D3D11_MAPPED_SUBRESOURCE MappedSubresource;
			VERIFYD3D11RESULT_EX(d3d_dc_->Map(StagingIndexBuffer,0,D3D11_MAP_READ,0,&MappedSubresource), d3d_device_);
			LockedData.SetData(MappedSubresource.pData);
			LockedData.Pitch = MappedSubresource.RowPitch;
		}
		else
...
void FD3D11DynamicRHI::RHIUnlockIndexBuffer(FIndexBufferRHIParamRef IndexBufferRHI)
{
	...
	FD3D11LockedKey LockedKey(IndexBuffer->Resource);
	assert(OutstandingLocks.count(LockedKey));
	...
	if(bIsDynamic)
	{
		...
		d3d_dc_->Unmap(IndexBuffer->Resource,0);
	}
	else
	{
		...
		else 
		{
			...
			d3d_dc_->UpdateSubresource(IndexBuffer->Resource,LockedKey.Subresource,NULL,LockedData->GetData(),LockedData->Pitch,0);
