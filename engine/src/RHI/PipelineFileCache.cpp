...
// Loaded + New created
#if STATS // If STATS are not enabled  will DLLEXPORT on an empty line
 DEFINE_STAT(STAT_TotalGraphicsPipelineStateCount);
 DEFINE_STAT(STAT_TotalComputePipelineStateCount);
#endif
...
//														   );
int32 CVarPSOFileCacheEnabled = PIPELINE_CACHE_DEFAULT_ENABLED;
...
//														   ECVF_Default | ECVF_RenderThreadSafe
//														   );
int32 CVarPSOFileCacheLogPSO = PIPELINE_CACHE_DEFAULT_ENABLED;
...
//														   ECVF_Default | ECVF_RenderThreadSafe
//														   );
int32 CVarPSOFileCacheReportPSO = PIPELINE_CACHE_DEFAULT_ENABLED;
...
//														   ECVF_Default
//														   );
int32 CVarPSOFileCachePrintNewPSODescriptors = GPSOFileCachePrintNewPSODescriptors;
...
//                                                            ECVF_Default | ECVF_RenderThreadSafe
//                                                            );
int CVarPSOFileCacheSaveUserCache = PIPELINE_CACHE_DEFAULT_ENABLED;

//FRWLock FPipelineFileCache::FileCacheLock;
FPipelineCacheFile* FPipelineFileCache::FileCache = nullptr;
std::map<uint32, uint32> FPipelineFileCache::RunTimeToFileHashes;
std::map<uint32, FPipelineStateStats*> FPipelineFileCache::Stats;
std::set<FPipelineCacheFileFormatPSO> FPipelineFileCache::NewPSOs;
...
std::string FPipelineFileCacheRasterizerState::ToString() const
{
	assert(0);
	return "";
	//return std::string::Printf(TEXT("<%f %f %u %u %u %u>")
	//	, DepthBias
	//	, SlopeScaleDepthBias
	//	, uint32(FillMode)
	//	, uint32(CullMode)
...
void FPipelineFileCacheRasterizerState::FromString(const std::string& InSrc)
{
...
std::string FPipelineCacheFileFormatPSO::ComputeDescriptor::ToString() const
{
	assert(0);
	return "";
	//return ComputeShader;
}
void FPipelineCacheFileFormatPSO::ComputeDescriptor::FromString(const std::string& Src)
{
	//ComputeShader.FromString(Src.TrimStartAndEnd());
}

std::string FPipelineCacheFileFormatPSO::ComputeDescriptor::HeaderLine()
{
	return std::string(("ComputeShader"));
}

std::string FPipelineCacheFileFormatPSO::GraphicsDescriptor::ShadersToString() const
{
	std::string Result;

	//Result += std::string::Printf(TEXT("%s,%s,%s,%s,%s")
...
void FPipelineCacheFileFormatPSO::GraphicsDescriptor::ShadersFromString(const std::string& Src)
{
	//std::vector<std::string> Parts;
	//Src.TrimStartAndEnd().ParseIntoArray(Parts, TEXT(","));
	//int32 PartIndex = 0;
...
std::string FPipelineCacheFileFormatPSO::GraphicsDescriptor::ShaderHeaderLine()
{
	return std::string(TEXT("VertexShader,FragmentShader,GeometryShader,HullShader,DomainShader"));
}

std::string FPipelineCacheFileFormatPSO::GraphicsDescriptor::StateToString() const
{
	std::string Result;
	return Result;
	//Result += std::string::Printf(TEXT("%s,%s,%s,")
	//	, *BlendState.ToString()
	//	, *RasterizerState.ToString()
	//	, *DepthStencilState.ToString()
	//);
	//Result += std::string::Printf(TEXT("%d,%d,%d,")
	//	, MSAASamples
	//	, uint32(DepthStencilFormat)
	//	, DepthStencilFlags
	//);
	//Result += std::string::Printf(TEXT("%d,%d,%d,%d,%d,")
	//	, uint32(DepthLoad)
	//	, uint32(StencilLoad)
	//	, uint32(DepthStore)
	//	, uint32(StencilStore)
	...
	//Result += std::string::Printf(TEXT("%d,")
	//	, RenderTargetsActive
	//);
	//for (int32 Index = 0; Index < MaxSimultaneousRenderTargets; Index++)
	//{
	//	Result += std::string::Printf(TEXT("%d,%d,%d,%d,")
	...
	//	{
	//		Result += std::string::Printf(TEXT("%s,")
...
void FPipelineCacheFileFormatPSO::GraphicsDescriptor::StateFromString(const std::string& Src)
{
	//std::vector<std::string> Parts;
	//Src.TrimStartAndEnd().ParseIntoArray(Parts, TEXT(","));
	//int32 PartIndex = 0;
...
std::string FPipelineCacheFileFormatPSO::GraphicsDescriptor::StateHeaderLine()
{
	std::string Result;
	return Result;
	//Result += std::string::Printf(TEXT("%s,%s,%s,")
	...
	//for (int32 Index = 0; Index < MaxSimultaneousRenderTargets; Index++)
	//{
	//	Result += std::string::Printf(TEXT("%s%d,%s%d,%s%d,%s%d,")
	...
	//for (int32 Index = 0; Index < MaxVertexElementCount; Index++)
	//{
	//	Result += std::string::Printf(TEXT("%s%d,")
...
std::string FPipelineCacheFileFormatPSO::GraphicsDescriptor::ToString() const
{
	return "";
	//return std::string::Printf(TEXT("%s,%s"), *ShadersToString(), *StateToString());
}

void FPipelineCacheFileFormatPSO::GraphicsDescriptor::FromString(const std::string& Src)
{
	//static const int32 NumShaderParts = 5;
	//std::vector<std::string> Parts;
	//Src.TrimStartAndEnd().ParseIntoArray(Parts, TEXT(","));
	//check(Parts.Num() > NumShaderParts);
	//std::vector<std::string> StateParts(Parts);
	//StateParts.RemoveAt(0, NumShaderParts);
	//Parts.RemoveAt(NumShaderParts, Parts.Num() - 5);
	//ShadersFromString(std::string::Join(Parts, TEXT(",")));
	//StateFromString(std::string::Join(StateParts, TEXT(",")));
}

std::string FPipelineCacheFileFormatPSO::GraphicsDescriptor::HeaderLine()
{
	//return std::string::Printf(TEXT("%s,%s"), *ShaderHeaderLine(), *StateHeaderLine());
	return "";
}
...
/*static*/ bool FPipelineCacheFileFormatPSO::Init(FPipelineCacheFileFormatPSO& PSO, FRHIComputeShader const* Init)
{
	...
	memset(&PSO.ComputeDesc, 0, sizeof(ComputeDescriptor));
...
/*static*/ bool FPipelineCacheFileFormatPSO::Init(FPipelineCacheFileFormatPSO& PSO, FGraphicsPipelineStateInitializer const& Init)
{
	...
	//
	//// Because of the cheat in the copy constructor - lets play this safe
	//memset(&PSO.GraphicsDesc, 0, sizeof(GraphicsDescriptor));
	//
...
, Hash(Other.Hash)
{
	switch(Type)
	{
		...
		{
			// If we implement a classic copy constructor without memcpy - remove memset in ::Init() function above
			memcpy(&ComputeDesc, &Other.ComputeDesc, sizeof(ComputeDescriptor));
			break;
		}
		case FPipelineCacheFileFormatPSO::DescriptorType::Graphics:
		{
			// If we implement a classic copy constructor without memcpy - remove memset in ::Init() function above
			memcpy(&GraphicsDesc, &Other.GraphicsDesc, sizeof(GraphicsDescriptor));
			break;
		}
		default:
		{
...
struct FPipelineCacheFileFormatTOC
{
	...
	std::unordered_map<uint32, FPipelineCacheFileFormatPSOMetaData> MetaData;
...
public:
	...
	//bool OpenPipelineFileCache(std::string const& FilePath, FGuid& Guid, TSharedPtr<IAsyncReadFileHandle, ESPMode::ThreadSafe>& Handle, FPipelineCacheFileFormatTOC& Content)
	//{
	//	bool bOK = false;
 ...
 //           FileReader->SetGameNetVer(FPipelineCacheFileFormatCurrentVersion);
	...
	bool OpenPipelineFileCache(std::string const& FileName, EShaderPlatform Platform)
	{
		return false;
...
//		
//
//		std::string GamePathStable = FPaths::ProjectContentDir() / TEXT("PipelineCaches") / ANSI_TO_TCHAR(FPlatformProperties::IniPlatformName()) / std::string::Printf(TEXT("%s_%s.stable.upipelinecache"), *FileName, *PlatformName.ToString());
//		std::string GamePath = FPaths::ProjectContentDir() / TEXT("PipelineCaches") / ANSI_TO_TCHAR(FPlatformProperties::IniPlatformName()) / std::string::Printf(TEXT("%s_%s.upipelinecache"), *FileName, *PlatformName.ToString());
//
//#if PLATFORM_MAC
...
//		}
//
//		std::string FilePath = FPaths::ProjectSavedDir() / std::string::Printf(TEXT("%s_%s.upipelinecache"), *FileName, *PlatformName.ToString());
//
//		RecordingFilename = std::string::Printf(TEXT("%s-CL-%u-"), *FEngineVersion::Current().GetBranchDescriptor(), FEngineVersion::Current().GetChangelist());
//
//		FGuid UniqueFileGuid;
//		FPlatformMisc::CreateGuid(UniqueFileGuid);  // not very unique on android, but won't matter much here
//		RecordingFilename += std::string::Printf(TEXT("%s_%s_%s.rec.upipelinecache"), *FileName, *PlatformName.ToString(), *UniqueFileGuid.ToString());
...
//
//		std::string JournalPath = FilePath + TEXT(".tmp");
...
//		return bGameFileOk || bUserFileOk;
	...
	bool SavePipelineFileCache(std::string const& FilePath, FPipelineFileCache::SaveMode Mode, std::unordered_map<uint32, FPipelineStateStats*> const& Stats, std::set<FPipelineCacheFileFormatPSO>& NewEntries, FPipelineFileCache::PSOOrder Order)
	{
		return false;
...
//			if (bPerformWrite)
//			{
//                std::string GamePathStable = FPaths::ProjectContentDir() / TEXT("PipelineCaches") / ANSI_TO_TCHAR(FPlatformProperties::IniPlatformName()) / std::string::Printf(TEXT("%s_%s.stable.upipelinecache"), *Name, *PlatformName.ToString());
//                std::string GamePath = FPaths::ProjectContentDir() / TEXT("PipelineCaches") / ANSI_TO_TCHAR(FPlatformProperties::IniPlatformName()) / std::string::Printf(TEXT("%s_%s.upipelinecache"), *Name, *PlatformName.ToString());
//#if PLATFORM_MAC
//                static bool bCommandLineNotStable = FParse::Param(FCommandLine::Get(), TEXT("nostablepipelinecache")) || IsRHIDeviceNVIDIA();
//#else
//				static bool bCommandLineNotStable = FParse::Param(FCommandLine::Get(), TEXT("nostablepipelinecache"));
...
//				std::vector<uint8> GameFileBytes;
//				int64 FileSize = IFileManager::Get().FileSize(*FilePath);
//				std::vector<uint8> UserFileBytes;
//				if(FPipelineFileCache::SaveMode::Incremental != Mode)
//				{
//					if (GameFileSize > 0)
//					{
...
//				if (bUseMemoryWriter)
//				{
//					FileWriter = new FMemoryWriter(SaveBytes, true, false, std::string(*SaveFilePath));
//				}
//				else
//				{
...
//									
//									std::vector<uint8> PSOData(Bytes, Entry.Value.FileSize);
...
//		return bFileWriteSuccess;
	...
	static void SortMetaData(std::unordered_map<uint32, FPipelineCacheFileFormatPSOMetaData>& MetaData, FPipelineFileCache::PSOOrder Order)
	{
	...
	void GetOrderedPSOHashes(std::vector<FPipelineCachePSOHeader>& PSOHashes, FPipelineFileCache::PSOOrder Order)
	{
	...
	void FetchPSODescriptors(std::vector<FPipelineCacheFileFormatPSORead*>& Batch)
	{
	...
	const std::string& GetRecordingFilename() const
	{
...
bool FPipelineFileCache::OpenPipelineFileCache(std::string const& Name, EShaderPlatform Platform)
{
	bool bOk = false;
	//
	//if(IsPipelineFileCacheEnabled())
...
bool FPipelineFileCache::SavePipelineFileCache(std::string const& Name, SaveMode Mode)
{
	bool bOk = false;
	...
	//	if(FileCache)
	//	{
	//		std::string PlatformName = FileCache->GetPlatformName();
	//		std::string Path = FPaths::ProjectSavedDir() / std::string::Printf(TEXT("%s_%s.upipelinecache"), *Name, *PlatformName.ToString());
...
void FPipelineFileCache::GetOrderedPSOHashes(std::vector<FPipelineCachePSOHeader>& PSOHashes, PSOOrder Order)
{
	//if(IsPipelineFileCacheEnabled())
	//{
	//	FRWScopeLock Lock(FileCacheLock, SLT_Write);
...
void FPipelineFileCache::FetchPSODescriptors(std::vector<FPipelineCacheFileFormatPSORead*>& Batch)
{
	//if(IsPipelineFileCacheEnabled())
	//{
	//	FRWScopeLock Lock(FileCacheLock, SLT_ReadOnly);
...
{
	FPipelineCacheFileFormatHeader Header;
	std::unordered_map<uint32, FPipelineCacheFileFormatPSO> PSOs;
	FPipelineCacheFileFormatTOC TOC;
	
	static FPipelineCacheFileData Open(std::string const& FilePath)
	{
...
bool FPipelineFileCache::LoadPipelineFileCacheInto(std::string const& Path, std::set<FPipelineCacheFileFormatPSO>& PSOs)
{
	return false;
...
bool FPipelineFileCache::SavePipelineFileCacheFrom(uint32 GameVersion, EShaderPlatform Platform, std::string const& Path, const std::set<FPipelineCacheFileFormatPSO>& PSOs)
{
...
bool FPipelineFileCache::MergePipelineFileCaches(std::string const& PathA, std::string const& PathB, FPipelineFileCache::PSOOrder Order, std::string const& OutputPath)
{
	return false;
	//bool bOK = false;
	//
