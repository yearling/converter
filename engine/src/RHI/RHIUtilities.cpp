...
//TAutoConsoleVariable<std::string> FDumpTransitionsHelper::CVarDumpTransitionsForResource(
...
std::string CVarDumpTransitionsForResource = "";
 FRHILockTracker GRHILockTracker;

std::string FDumpTransitionsHelper::DumpTransitionForResource = "";


void FDumpTransitionsHelper::DumpTransitionForResourceHandler()
{
	const std::string NewValue = CVarDumpTransitionsForResource/*.GetValueOnGameThread()*/;
	DumpTransitionForResource = std::string(NewValue);
}

std::string FDumpTransitionsHelper::CVarDumpTransitionsForResource = "";

void FDumpTransitionsHelper::DumpResourceTransition(const std::string& ResourceName, const EResourceTransitionAccess TransitionType)
{
	const std::string ResourceDumpName = FDumpTransitionsHelper::DumpTransitionForResource;
	if ((ResourceDumpName != "") && (ResourceDumpName == ResourceName))
	{
		const uint32 DumpCallstackSize = 2047;
		char DumpCallstack[DumpCallstackSize] = { 0 };
...
void SetDepthBoundsTest(FRHICommandList& RHICmdList, float WorldSpaceDepthNear, float WorldSpaceDepthFar, const YMatrix& ProjectionMatrix)
{
	if (GSupportsDepthBoundsTest)
	{
		YVector4 Near = ProjectionMatrix.TransformVector4(YVector4(0, 0, WorldSpaceDepthNear,1.0));
		YVector4 Far = ProjectionMatrix.TransformVector4(YVector4(0, 0, WorldSpaceDepthFar,1.0));
		float DepthNear = Near.z / Near.w;
		float DepthFar = Far.z / Far.w;

		DepthFar = YMath::Clamp(DepthFar, 0.0f, 1.0f);
		DepthNear = YMath::Clamp(DepthNear, 0.0f, 1.0f);
...
//	ECVF_Default
//);
float CVarRHIPresentThresholdTop = 0.0;
...
//	ECVF_Default
//);
float CVarRHIPresentThresholdBottom = 0.0;
...
int32 CVarRHISyncAllowEarlyKick = 1;
...
class FRHIFrameFlipTrackingRunnable : public FRunnable
{
	...
	std::vector<FFramePair> FramePairs;
...
struct FRHIFrameOffsetThread : public FRunnable
{
	...
	virtual uint32 Run() override
	{
		while (bRun)
		{
			...
			double Timeout = YMath::Max(0.0, TargetFlipTime - FPlatformTime::Seconds());
...
uint32 FRHIFrameFlipTrackingRunnable::Run()
{
	...
	while (bRun)
	{
		...
		double TimeoutInSeconds = (SyncInterval == 0 || bForceFlipSync) ? -1.0 : YMath::Max(ExpectedNextFlipTimeInSeconds - CurrentTimeInSeconds, 0.0);
		...
		FScopeLock Lock(&CS);
		for (int32 PairIndex = FramePairs.size() - 1; PairIndex >= 0; --PairIndex)
		{
			...
			if (Pair.PresentIndex <= SyncFrame)
			{
				...
				FramePairs.erase(FramePairs.begin() + PairIndex);
			}
		}
	}
...
void FRHIFrameFlipTrackingRunnable::CompleteGraphEventOnFlip(uint64 PresentIndex, FGraphEventRef Event)
{
	...
	if (Thread)
	{
		...
		Singleton.FramePairs.push_back(Pair);
...
{
	OutTopPercent = YMath::Clamp(CVarRHIPresentThresholdTop/*.GetValueOnAnyThread()*/, 0.0f, 1.0f);
	OutBottomPercent = YMath::Clamp(CVarRHIPresentThresholdBottom/*.GetValueOnAnyThread()*/, 0.0f, 1.0f);
}
