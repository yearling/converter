...
//	ECVF_Default);
//
static std::string GProfileGPUPatternCVar = "";
...
//	ECVF_Default);
//
static std::string GProfileGPURootCVar = "";
...
//	ECVF_Default);
//
static int32 GProfileShowEventHistogram = 0;
...
static int32 GProfileGPUShowEvents = 0;
...
//	ECVF_Default);
//
static int32 GProfileGPUTransitions = 0;
...
static int32 GProfilePrintAssetSummary = 0;
...
static std::string GProfileAssetSummaryCallOuts = "";
...
//	ECVF_RenderThreadSafe);
static int32 CVarGPUCrashDataCollectionEnable = 1;
...
static int32 GProfileGPUSort = 0;
...
static void GatherStatsEventNode(FGPUProfilerEventNode* Node, int32 Depth, std::unordered_map<std::string, FGPUProfilerEventNodeStats>& EventHistogram)
{
	if (Node->NumDraws > 0 || Node->Children.size() > 0)
	{
		...
		for (int32 ChildIndex = 0; ChildIndex < Node->Children.size(); ChildIndex++)
		{
			// Traverse children
		...
		bool find_count = EventHistogram.count(Node->Name);
		if (find_count)
		{
			FGPUProfilerEventNodeStats* FoundHistogramBucket = &EventHistogram[Node->Name];
		...
		else
		{
			...
			EventHistogram[Node->Name] =  NewNodeStats;
		}
	}
}
...
{
	std::unordered_map<std::string, FGPUProfileInfoPair> TrianglesPerMaterial;
	std::unordered_map<std::string, FGPUProfileInfoPair> TrianglesPerMesh;
	std::unordered_map<std::string, FGPUProfileInfoPair> TrianglesPerNonMesh;
	...
	{
		bDumpEventLeafNodes = GProfileGPUShowEvents != 0;
		bGatherSummaryStats = GProfilePrintAssetSummary != 0;
	}

	void ProcessMatch(FGPUProfilerEventNode* Node)
	{
		if (bGatherSummaryStats && (Node->NumTotalPrimitives > 0) && (Node->NumTotalVertices > 0) && (Node->Children.size() == 0))
		{
...
static void DumpStatsEventNode(FGPUProfilerEventNode* Node, float RootResult, int32 Depth, const std::string& WildcardFilter, bool bParentMatchedFilter, FGPUProfileStatSummary& Summary)
{
	...
	//	const int32 EffectiveDepth = YMath::Max(Depth - 1, 0);
...
void FGPUProfiler::PushEvent(const TCHAR* Name, FColor Color)
{
	if (bTrackingEvents)
	{
		...
		if (CurrentEventNode)
		{
			...
			CurrentEventNode->Children.push_back(CreateEventNode(Name, CurrentEventNode));
			CurrentEventNode = CurrentEventNode->Children.back().GetReference();
		}
		else
		{
			// Add a new root node to the tree
			CurrentEventNodeFrame->EventTree.push_back(CreateEventNode(Name, NULL));
			CurrentEventNode = CurrentEventNodeFrame->EventTree.back().GetReference();
		}
...
{
	if (QueriesFinished < StartQueries.size())
	{
		return 0.0f;
	}
	...
	//
	for (int32 i = StartQueries.size() - WindowSize; i < StartQueries.size(); i++)
	{
		...
		QueriesFailed = YMath::Max<int32>(0, QueriesFailed);
