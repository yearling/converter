...
static void RHIDetectAndWarnOfBadDrivers()
{
	...
	int32 CVarValue = CVarWarnOfBadDrivers;
	...
	if (DriverInfo.IsValid())
	{
		...
		if (BlackListEntry.IsValid())
		{
			...
			std::string VendorString = DriverInfo.ProviderName;
...
void RHIPostInit(const std::vector<uint32>& InPixelFormatByteWidth)
{
	check(GDynamicRHI);
...
static void BaseRHISetGPUCaptureOptions(const std::vector<std::string>& Args/*, UWorld* World*/)
{
	/*if (Args.Num() > 0)
	{
...
void FDynamicRHI::EnableIdealGPUCaptureOptions(bool bEnabled)
{
	...
	//	Args.Add(std::string::Printf(TEXT("%i"), bShouldEnableRHIThread ? 1 : 0));
