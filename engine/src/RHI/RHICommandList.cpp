...
#include "RHI/RHICommandListCommandExecutes.inl"
#include "Platform/Windows/YWindowsPlatformTime.h"
...
int32 CVarRHICmdBypass = FRHICommandListExecutor::DefaultBypass;

//static TAutoConsoleVariable<int32> CVarRHIRenderPassValidation(
//	TEXT("r.RenderPass.Validation"),
//	0,
//	TEXT(""));
int32 CVarRHIRenderPassValidation = 0;
...
int32 CVarRHICmdUseParallelAlgorithms = 1;
...
int32 CVarRHICmdUseDeferredContexts = 1;
...
int32 CVarRHICmdFlushRenderThreadTasks = 0;
...
int32 CVarRHICmdFlushUpdateTextureReference = 0;
...
int32 CVarRHICmdFlushOnQueueParallelSubmit = 0;
...
int32 CVarRHICmdMergeSmallDeferredContexts = 1;
...
int32 CVarRHICmdBufferWriteLocks = 1;
...
int32 CVarRHICmdAsyncRHIThreadDispatch = 1;
...
int32 CVarRHICmdCollectRHIThreadStatsFromHighLevel = 1;
...
int32 CVarRHICmdForceRHIFlush = 0;
...
int32 CVarRHICmdBalanceTranslatesAfterTasks = 0;
...
int32 CVarRHICmdMinCmdlistForParallelTranslate = 2;
...
int32 CVarRHICmdMinCmdlistSizeForParallelTranslate = 32;
...
FRHICommandListFenceAllocator GRHIFenceAllocator;
...
{
	return !!WaitOutstandingTasks.size();
}
...
ENamedThreads::Type CPrio_RHIThreadOnTaskThreads = ENamedThreads::Type(ENamedThreads::NormalThreadPriority | ENamedThreads::NormalTaskPriority | ENamedThreads::AnyThread);
...
public:
	...
	ENamedThreads::Type GetDesiredThread()
	{
		...
		return IsRunningRHIInDedicatedThread() ? ENamedThreads::RHIThread : CPrio_RHIThreadOnTaskThreads;
	}
...
public:
	...
	ENamedThreads::Type GetDesiredThread()
	{
		...
		check(IsRunningRHIInSeparateThread()); // this should never be used on a platform that doesn't support the RHI thread
		return bRHIThread ? (IsRunningRHIInDedicatedThread() ? ENamedThreads::RHIThread : CPrio_RHIThreadOnTaskThreads) : ENamedThreads::GetRenderThread_Local();
	}
	...
	void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
	{
		...
		{
			Prereq.push_back(RHIThreadTask);
		}
...
void FRHICommandListExecutor::ExecuteInner(FRHICommandListBase& CmdList)
{
	...
	if (IsRunningRHIInSeparateThread())
	{
		...
		if (CVarRHICmdUseThread/*.GetValueOnRenderThread()*/ > 0 && bIsInRenderingThread && !bIsInGameThread)
		{
			...
			FGraphEventArray Prereq;
			std::swap(Prereq, CmdList.RTTasks);
			{
				//QUICK_SCOPE_CYCLE_COUNTER(STAT_FRHICommandListExecutor_SwapCmdLists);
			...
			{
				Prereq.insert(Prereq.begin(),AllOutstandingTasks.begin(),AllOutstandingTasks.end());
				AllOutstandingTasks.clear();
				if (RenderThreadSublistDispatchTask.GetReference())
				{
					Prereq.push_back(RenderThreadSublistDispatchTask);
				}
			...
			if (CVarRHICmdForceRHIFlush> 0)
			{
				if (FTaskGraphInterface::Get().IsThreadProcessingTasks(RenderThread_Local))
				{
					...
					ERROR_INFO("Deadlock in FRHICommandListExecutor::ExecuteInner 2.");
				}
				if (RenderThreadSublistDispatchTask.GetReference())
				{
		...
		if (bIsInRenderingThread)
		{
			...
			if (RenderThreadSublistDispatchTask.GetReference())
			{
				if (FTaskGraphInterface::Get().IsThreadProcessingTasks(RenderThread_Local))
				{
					...
					ERROR_INFO("Deadlock in FRHICommandListExecutor::ExecuteInner (RenderThreadSublistDispatchTask).");
				}
				FTaskGraphInterface::Get().WaitUntilTaskCompletes(RenderThreadSublistDispatchTask, RenderThread_Local);
				RenderThreadSublistDispatchTask = nullptr;
			}
			while (RHIThreadTask.GetReference())
			{
				if (FTaskGraphInterface::Get().IsThreadProcessingTasks(RenderThread_Local))
				{
					...
					ERROR_INFO("Deadlock in FRHICommandListExecutor::ExecuteInner (RHIThreadTask).");
				}
				FTaskGraphInterface::Get().WaitUntilTaskCompletes(RHIThreadTask, RenderThread_Local);
				if (RHIThreadTask.GetReference() && RHIThreadTask->IsComplete())
				{
...
struct FRHICommandRHIThreadFence final : public FRHICommand<FRHICommandRHIThreadFence>
{
	...
	{
		check(IsInRHIThread());
		static std::vector<FBaseGraphTask*> NewTasks;
...
void FRHICommandListExecutor::WaitOnRHIThreadFence(FGraphEventRef& Fence)
{
	...
	if (Fence.GetReference() && !Fence->IsComplete())
	{
		...
		if (FTaskGraphInterface::Get().IsThreadProcessingTasks(RenderThread_Local))
		{
			...
			ERROR_INFO("Deadlock in WaitOnRHIThreadFence.");
		}
		FTaskGraphInterface::Get().WaitUntilTaskCompletes(Fence, RenderThread_Local);
	}
}
...
//	ENamedThreads::NormalTaskPriority
//);
ENamedThreads::Type CPrio_FParallelTranslateCommandList = ENamedThreads::Type(ENamedThreads::NormalThreadPriority | ENamedThreads::NormalTaskPriority | ENamedThreads::AnyThread);
...
//	ENamedThreads::HighTaskPriority
//);
ENamedThreads::Type CPrio_FParallelTranslateCommandListPrepass = ENamedThreads::Type(ENamedThreads::NormalThreadPriority | ENamedThreads::HighTaskPriority | ENamedThreads::AnyThread);
...
public:
	...
	ENamedThreads::Type GetDesiredThread()
	{
		return bIsPrepass ? CPrio_FParallelTranslateCommandListPrepass: CPrio_FParallelTranslateCommandList;
	}
...
ENamedThreads::Type CPrio_FParallelTranslateSetupCommandList = ENamedThreads::Type(ENamedThreads::HighThreadPriority | ENamedThreads::HighTaskPriority | ENamedThreads::AnyThread);
...
public:
	...
	{
		return CPrio_FParallelTranslateSetupCommandList;
	}
	...
	void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
	{
		...
		for (int32 Index = 0; Index < NumCommandLists; Index++)
		{
			Sizes.push_back(RHICmdLists[Index]->GetUsedMemory());
		}
		...
		else
		{
			...
			while (Start < NumCommandLists)
			{
				...
				MyCompletionGraphEvent->SetGatherThreadForDontCompleteUntil(ENamedThreads::Type(CPrio_FParallelTranslateCommandList | ENamedThreads::HighTaskPriority));
	...
	if (Num && IsRunningRHIInSeparateThread())
	{
		...
		if (/*ICVarRHICmdBalanceParallelLists->GetValueOnRenderThread() == 0 && */CVarRHICmdBalanceTranslatesAfterTasks/*.GetValueOnRenderThread() */> 0 && GRHISupportsParallelRHIExecute && CVarRHICmdUseDeferredContexts/*.GetValueOnAnyThread()*/ > 0)
		{
			...
			for (int32 Index = 0; Index < Num; Index++)
			{
				...
				{
					Prereq.push_back(AnyThreadCompletionEvent);
					WaitOutstandingTasks.push_back(AnyThreadCompletionEvent);
				}
			}
			...
			{
				Prereq.push_back(RHIThreadBufferLockFence);
			}
			...
			AllOutstandingTasks.push_back(TranslateSetupCompletionEvent);
			if (IsRunningRHIInSeparateThread())
			{
		...
		if (ContextContainer)
		{
			while (Start < Num)
			{
				...
				for (int32 Index = Start; Index <= Last; Index++)
				{
					...
					{
						Prereq.push_back(AnyThreadCompletionEvent);
						AllOutstandingTasks.push_back(AnyThreadCompletionEvent);
						WaitOutstandingTasks.push_back(AnyThreadCompletionEvent);
					}
				}
				...
				AllOutstandingTasks.push_back(TranslateCompletionEvent);
			...
			if (EventToWaitFor.GetReference() && !EventToWaitFor->IsComplete())
			{
				...
				if (IsInRenderingThread())
				{
					...
					if (FTaskGraphInterface::Get().IsThreadProcessingTasks(RenderThread_Local))
					{
						...
						ERROR_INFO("Deadlock in command list processing.");
					}
...
void FRHICommandListBase::QueueRenderThreadCommandListSubmit(FGraphEventRef& RenderThreadCompletionEvent, class FRHICommandList* CmdList)
{
	...
	if (RenderThreadCompletionEvent.GetReference())
	{
		...
		RTTasks.push_back(RenderThreadCompletionEvent);
	}
...
void FRHICommandListBase::QueueAsyncPipelineStateCompile(FGraphEventRef& AsyncCompileCompletionEvent)
{
	if (AsyncCompileCompletionEvent.GetReference())
	{
		...
		auto result = std::find(RTTasks.begin(), RTTasks.end(), AsyncCompileCompletionEvent);
		if (result != RTTasks.end())
		{
			RTTasks.push_back(AsyncCompileCompletionEvent);
		}
	}
}
...
{
	check(IsImmediate() && IsInRenderingThread());
	check(!AllOutstandingTasks.size()); // dispatch before you get here
	if (RenderThreadSublistDispatchTask.GetReference() && RenderThreadSublistDispatchTask->IsComplete())
	{
		RenderThreadSublistDispatchTask = nullptr;
	}
	while (RenderThreadSublistDispatchTask.GetReference())
	{
		...
		if (FTaskGraphInterface::Get().IsThreadProcessingTasks(RenderThread_Local))
		{
			...
			ERROR_INFO("Deadlock in FRHICommandListBase::WaitForDispatch.");
		}
		FTaskGraphInterface::Get().WaitUntilTaskCompletes(RenderThreadSublistDispatchTask, RenderThread_Local);
		if (RenderThreadSublistDispatchTask.GetReference() && RenderThreadSublistDispatchTask->IsComplete())
		{
...
void FRHICommandListBase::HandleRTThreadTaskCompletion(const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	for (int32 Index = 0; Index < RTTasks.size(); Index++)
	{
...
FVertexShaderRHIRef FDynamicRHI::CreateVertexShader_RenderThread(class FRHICommandListImmediate& RHICmdList, const std::vector<uint8>& Code)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
FPixelShaderRHIRef FDynamicRHI::CreatePixelShader_RenderThread(class FRHICommandListImmediate& RHICmdList, const std::vector<uint8>& Code)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
FGeometryShaderRHIRef FDynamicRHI::CreateGeometryShader_RenderThread(class FRHICommandListImmediate& RHICmdList, const std::vector<uint8>& Code)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
FGeometryShaderRHIRef FDynamicRHI::CreateGeometryShaderWithStreamOutput_RenderThread(class FRHICommandListImmediate& RHICmdList, const std::vector<uint8>& Code, const FStreamOutElementList& ElementList, uint32 NumStrides, const uint32* Strides, int32 RasterizedStream)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
FComputeShaderRHIRef FDynamicRHI::CreateComputeShader_RenderThread(class FRHICommandListImmediate& RHICmdList, const std::vector<uint8>& Code)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
FHullShaderRHIRef FDynamicRHI::CreateHullShader_RenderThread(class FRHICommandListImmediate& RHICmdList, const std::vector<uint8>& Code)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
FDomainShaderRHIRef FDynamicRHI::CreateDomainShader_RenderThread(class FRHICommandListImmediate& RHICmdList, const std::vector<uint8>& Code)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
FUpdateTexture3DData FDynamicRHI::BeginUpdateTexture3D_RenderThread(class FRHICommandListImmediate& RHICmdList, FTexture3DRHIParamRef Texture, uint32 MipIndex, const struct FUpdateTextureRegion3D& UpdateRegion)
{
	...
	uint8* Data = (uint8*)malloc(MemorySize);
...
FRHIShaderLibraryRef FDynamicRHI::RHICreateShaderLibrary_RenderThread(class FRHICommandListImmediate& RHICmdList, EShaderPlatform Platform, std::string FilePath, std::string Name)
{
	FScopedRHIThreadStaller StallRHIThread(RHICmdList);
...
void FDynamicRHI::RHIReadSurfaceFloatData_RenderThread(class FRHICommandListImmediate& RHICmdList, FTextureRHIParamRef Texture, FIntRect Rect,std::vector<FFloat16Color>& OutData, ECubeFace CubeFace, int32 ArrayIndex, int32 MipIndex)
{
