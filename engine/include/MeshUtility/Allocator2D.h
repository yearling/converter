...
public:
	...
	struct FRun
	{
		...
		std::vector< uint16 > FreeSegmentsLookup;
	};
...
public:
	...
	bool       FindWithSegments( FRect& Rect, const FAllocator2D& Other, std::function<bool (const FAllocator2D::FRect&)> IsBestRect ) const;
	...
	void       CopyRuns( std::vector<FRun>& Runs, const std::vector<FRun>& OtherRuns, int32 MaxSize );
	...
	void       ResetStats();
	void       PublishStats( int32 ChartIndex, int32 Orientation, bool bFound, const FRect& Rect, const FRect& BestRect, const FMD5Hash& ChartMD5, std::function<bool (const FAllocator2D::FRect&)> IsBestRect );
...
protected:
	...
	void       InitRuns( std::vector<FRun>& Runs, uint32 PrimaryResolution, uint32 PerpendicularRasterSize);
	void       InitSegments();
	...
	uint32                   RasterWidth;
	uint32                   RasterHeight;
	std::vector< FRun >           Rows;        // Represent rows in the grid
	std::vector< FRun >           Columns;     // Represent columns in the grid (used when version >= Segments2D).
	std::vector< uint64 >         Bits;

	// Index inside rows that will be sorted by rows with longest used segment first
	std::vector< uint16 >         SortedRowsIndex;
	// Index inside columns that will be sorted by columns with longest used segment first
	std::vector< uint16 >         SortedColumnsIndex;
