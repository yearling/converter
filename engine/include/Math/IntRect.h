...
public:
	 ...
	 */
	const FIntPoint& operator()(int32_t PointIndex) const;
	 ...
	 */
	FIntPoint& operator()(int32_t PointIndex);
	 ...
	 */
	FIntRect& operator*=(int32_t Scale);
	 ...
	 */
	FIntRect operator*(int32_t Scale) const;
	 ...
	 */
	FIntRect operator/(int32_t Div) const;
...
public:
	 ...
	 */
	FIntRect Bottom(int32_t InHeight) const;
	 ...
	 */
	void InflateRect(int32_t Amount);
	 ...
	 * @return The new rectangle.
	 */
	FIntRect Right(int32_t InWidth) const;
...
public:
	 ...
	 * @return New divided rectangle.
	 */
	static FIntRect DivideAndRoundUp(FIntRect lhs, int32_t Div);
...
inline FIntRect::FIntRect(int32_t X0, int32_t Y0, int32_t X1, int32_t Y1)
	: Min(X0, Y0)
	, Max(X1, Y1)
{ }
...
inline const FIntPoint& FIntRect::operator()(int32_t PointIndex) const
{
	return (&Min)[PointIndex];
}


inline FIntPoint& FIntRect::operator()(int32_t PointIndex)
{
	return (&Min)[PointIndex];
}
...
inline FIntRect& FIntRect::operator*=(int32_t Scale)
{
	Min *= Scale;
	Max *= Scale;
...
inline FIntRect FIntRect::operator*(int32_t Scale) const
{
	return FIntRect(Min * Scale, Max * Scale);
}


inline FIntRect FIntRect::operator/(int32_t Div) const
{
...
inline int32_t FIntRect::Area() const
{
	return (Max.X - Min.X) * (Max.Y - Min.Y);
}


inline FIntRect FIntRect::Bottom(int32_t InHeight) const
{
	return FIntRect(Min.X, YMath::Max(Min.Y, Max.Y - InHeight), Max.X, Max.Y);
}


inline void FIntRect::Clip(const FIntRect& R)
{
	Min.X = YMath::Max<int32_t>(Min.X, R.Min.X);
	Min.Y = YMath::Max<int32_t>(Min.Y, R.Min.Y);
	Max.X = YMath::Min<int32_t>(Max.X, R.Max.X);
	Max.Y = YMath::Min<int32_t>(Max.Y, R.Max.Y);

	// return zero area if not overlapping
	Max.X = YMath::Max<int32_t>(Min.X, Max.X);
	Max.Y = YMath::Max<int32_t>(Min.Y, Max.Y);
}

inline void FIntRect::Union(const FIntRect& R)
{
	Min.X = YMath::Min<int32_t>(Min.X, R.Min.X);
	Min.Y = YMath::Min<int32_t>(Min.Y, R.Min.Y);
	Max.X = YMath::Max<int32_t>(Max.X, R.Max.X);
	Max.Y = YMath::Max<int32_t>(Max.Y, R.Max.Y);
}
...
inline FIntRect FIntRect::DivideAndRoundUp(FIntRect lhs, int32_t Div)
{
...
inline int32_t FIntRect::Height() const
{
	return (Max.Y - Min.Y);
}


inline void FIntRect::InflateRect(int32_t Amount)
{
	Min.X -= Amount;
	Min.Y -= Amount;
	Max.X += Amount;
...
inline int32_t FIntRect::Num()
{
	return 2;
}


inline FIntRect FIntRect::Right(int32_t InWidth) const
{
	return FIntRect(YMath::Max(Min.X, Max.X - InWidth), Min.Y, Max.X, Max.Y);
}
...
inline int32_t FIntRect::Width() const
{
	return Max.X - Min.X;
}
