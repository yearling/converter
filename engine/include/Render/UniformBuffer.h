	...
	)
{
	...
	TUniformBuffer< TBufferStruct>* uniform_buffer = &UniformBuffer;
	ENQUEUE_RENDER_COMMAND(SetUniformBufferContents, [=Struct](FRHICommandListImmediate& RHICmdList) {
		uniform_buffer->SetContents(Struct);
		});
}
...
public:
	...
	virtual ~FUniformBufferStruct()
	{
		...
		//GetNameStructMap().Remove(std::string(StructTypeName, std::string_Find));
		GetNameStructMap().erase(StructTypeName);
	}

	void InitializeLayout();

	void GetNestedStructs(std::vector<const FUniformBufferStruct*>& OutNestedStructs) const;

	void AddResourceTableEntries(std::unordered_map<std::string, FResourceTableEntry>& ResourceTableMap, std::unordered_map<std::string, uint32>& ResourceTableLayoutHashes) const;
...
private:
	...
	void AddResourceTableEntriesRecursive(const char* UniformBufferName, const char* Prefix, uint16& ResourceIndex, std::unordered_map<std::string, FResourceTableEntry>& ResourceTableMap) const;
};
...
//
	...
	template<>
	class TUniformBufferTypeInfo<YVector2, false>
	{
	public:
		enum { BaseType = UBMT_FLOAT32 };
		enum { NumRows = 1 };
		...
		enum { Alignment = 8 };
		enum { IsResource = 0 };
		typedef TAlignedTypedef<YVector2,Alignment>::TAlignedType TAlignedType;
		static const FUniformBufferStruct* GetStruct() { return NULL; }
	};

	template<>
	class TUniformBufferTypeInfo<YVector, false>
	{
	public:
		enum { BaseType = UBMT_FLOAT32 };
		enum { NumRows = 1 };
		...
		enum { Alignment = 16 };
		enum { IsResource = 0 };
		typedef TAlignedTypedef<YVector,Alignment>::TAlignedType TAlignedType;
		static const FUniformBufferStruct* GetStruct() { return NULL; }
	};

	template<>
	class TUniformBufferTypeInfo<YVector4, false>
	{
	public:
		enum { BaseType = UBMT_FLOAT32 };
		enum { NumRows = 1 };
		...
		enum { Alignment = 16 };
		enum { IsResource = 0 };
		typedef TAlignedTypedef<YVector4,Alignment>::TAlignedType TAlignedType;
		static const FUniformBufferStruct* GetStruct() { return NULL; }
	};
	...
	template<>
	class TUniformBufferTypeInfo<YMatrix, false>
	{
	public:
		enum { BaseType = UBMT_FLOAT32 };
		enum { NumRows = 4 };
		...
		enum { Alignment = 16 };
		enum { IsResource = 0 };
		typedef TAlignedTypedef<YMatrix,Alignment>::TAlignedType TAlignedType;
		static const FUniformBufferStruct* GetStruct() { return NULL; }
	};
...
#define IMPLEMENT_UNIFORM_BUFFER_STRUCT(StructTypeName,ShaderVariableName) \
	...
	std::string(TEXT(#StructTypeName)), \
	TEXT(#StructTypeName), \
	ShaderVariableName, \
...
#define DECLARE_UNIFORM_BUFFER_STRUCT_MEMBER_EXPLICIT(MemberType,MemberName,ArrayDecl,Precision,OptionalShaderType,IsMemberStruct) \
	...
	private: \
		...
		{ \
			...
			static_assert(TUniformBufferTypeInfo<zzT##MemberName, IsMemberStruct>::IsResource == 0 || TIsArrayOrRefOfType<decltype(OptionalShaderType), char>::Value, "No shader type for " #MemberName "."); \
...
#define END_UNIFORM_BUFFER_STRUCT(Name) \
			zzLastMemberId; \
		static std::vector<FUniformBufferStruct::FMember> zzGetMembers() { return zzGetMembersBefore(zzLastMemberId()); } \
...
extern  FUniformBufferStruct* FindUniformBufferStructByName(const char* StructName);
//extern  FUniformBufferStruct* FindUniformBufferStructBystd::string(std::string StructName);
