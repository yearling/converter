...
* This is called in the game thread.
*/
extern  void StartBatchedRelease();

/**
* Disables the batching of calls to BeginReleaseResource
* This is called in the game thread.
*/
extern  void EndBatchedRelease();
...
struct FMipBiasFade
{
	...
	inline float	CalcMipBias() const
	{
		...
		float TimeFactor	= YMath::Min<float>(DeltaTime * MipCountFadingRate, 1.0f);
		float MipBias		= BiasOffset - MipCountDelta*TimeFactor;
		return YMath::FloatSelect(GEnableMipLevelFading, MipBias, 0.0f);
	}
	...
	inline bool	IsFading( ) const
	{
		...
		return (YMath::Abs<float>(MipCountDelta) > SMALL_NUMBER && TimeFactor < 1.0f);
	}
};
...
public:
	...
	virtual std::string GetFriendlyName() const override { return TEXT("FTexture"); }
};
...
public:
	...
	virtual std::string GetFriendlyName() const override { return TEXT("FVertexBuffer"); }
};
...
public:
	...
	virtual std::string GetFriendlyName() const override { return TEXT("FIndexBuffer"); }
};
