...
namespace std
{
	...
	template <>
	struct hash<FBoundShaderStateLookupKey>
	{
		size_t operator()(const FBoundShaderStateLookupKey& k) const
		{
 ...
 * @return If a bound shader state matching the parameters is cached, it is returned; otherwise NULL is returned.
 */
extern  FBoundShaderStateRHIRef GetCachedBoundShaderState_Threadsafe(
	FVertexDeclarationRHIParamRef VertexDeclaration,
	FVertexShaderRHIParamRef VertexShader,
	FPixelShaderRHIParamRef PixelShader,
	FHullShaderRHIParamRef HullShader = NULL,
