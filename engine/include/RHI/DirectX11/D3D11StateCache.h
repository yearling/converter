...
protected:
	...
	FD3D11VertexBufferState CurrentVertexBuffers[D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT];
	...
	FD3D11ConstantBufferState CurrentConstantBuffers[SF_NumFrequencies][D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT];
...
public:
	...
	{
#if D3D11_ALLOW_STATE_CACHE
		memset(&CurrentConstantBuffers[ShaderFrequency], 0, sizeof(FD3D11ConstantBufferState));
#endif
	...
	D3D11_STATE_CACHE_INLINE void GetViewports(uint32* Count, D3D11_VIEWPORT* Viewports)
	{
		...
		if (Viewports) //NULL is legal if you just want count
		{
			...
			int32 CopyCount = YMath::Min(YMath::Min(StorageSizeCount, (int32)CurrentNumberOfViewports), D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE);
			if (CopyCount > 0)
			{
	...
	D3D11_STATE_CACHE_INLINE void GetConstantBuffers(uint32 StartSlotIndex, uint32 NumBuffers, ID3D11Buffer** ConstantBuffers)
	{
...
#else
		{
			switch (ShaderFrequency)
			{
			case SF_Vertex:		Direct3DDeviceIMContext->VSGetConstantBuffers(StartSlot, NumBuffers, ConstantBuffers); break;
			case SF_Hull:		Direct3DDeviceIMContext->HSGetConstantBuffers(StartSlot, NumBuffers, ConstantBuffers); break;
			case SF_Domain:		Direct3DDeviceIMContext->DSGetConstantBuffers(StartSlot, NumBuffers, ConstantBuffers); break;
			case SF_Geometry:	Direct3DDeviceIMContext->GSGetConstantBuffers(StartSlot, NumBuffers, ConstantBuffers); break;
			case SF_Pixel:		Direct3DDeviceIMContext->PSGetConstantBuffers(StartSlot, NumBuffers, ConstantBuffers); break;
			case SF_Compute:	Direct3DDeviceIMContext->CSGetConstantBuffers(StartSlot, NumBuffers, ConstantBuffers); break;
...
public:
		...
		: Direct3DDeviceIMContext(nullptr)
	{
		...
		memset(&CurrentShaderResourceViews, 0, sizeof(CurrentShaderResourceViews));
	}
