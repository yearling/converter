...
{
public:
	static constexpr uint32_t GetEndOfStreamToken()
	{
		return 0xffffffff;
	}

	static uint32_t Create(uint16_t UniformBufferIndex, uint16_t ResourceIndex, uint16_t BindIndex)
	{
	...
	static inline uint16_t GetUniformBufferIndex(uint32_t Data)
	{
		return (Data >> RTD_Shift_UniformBufferIndex) & RTD_Mask_UniformBufferIndex;
	}

	static inline uint16_t GetResourceIndex(uint32_t Data)
	{
		return (Data >> RTD_Shift_ResourceIndex) & RTD_Mask_ResourceIndex;
	}

	static inline uint16_t GetBindIndex(uint32_t Data)
	{
...
private:
	...
	static_assert(RTD_NumBits_UniformBufferIndex + RTD_NumBits_ResourceIndex + RTD_NumBits_BindIndex <= sizeof(uint32_t)* 8, "RTD_* values must fit in 32 bits");
};
...
enum EBufferUsageFlags
{
	...
	/** Create a byte address buffer, which is basically a structured buffer with a uint32_t type. */
...
inline ERHIFeatureLevel::Type GetMaxSupportedFeatureLevel(EShaderPlatform InShaderPlatform)
{
	...
	default:
		assert(0, ("Unknown ShaderPlatform %d"), (int32_t)InShaderPlatform);
...
inline uint32_t GetExpectedFeatureLevelMaxTextureSamplers(ERHIFeatureLevel::Type FeatureLevel)
{
...
inline int32_t GetFeatureLevelMaxNumberOfBones(ERHIFeatureLevel::Type FeatureLevel)
{
	switch (FeatureLevel)
	{
	...
	case ERHIFeatureLevel::SM5:
		return 256;
	default:
		assert(0, ("Unknown FeatureLevel %d"), (int32_t)FeatureLevel);
	}
...
{
	const char* String = "SF_NumFrequencies";
	switch (Frequency)
	{
	case SF_Vertex:			String = "SF_Vertex"; break;
	case SF_Hull:			String = "SF_Hull"; break;
	case SF_Domain:			String = "SF_Domain"; break;
	case SF_Geometry:		String = "SF_Geometry"; break;
	case SF_Pixel:			String = "SF_Pixel"; break;
	case SF_Compute:		String = "SF_Compute"; break;
	default:				
		assert(0, TEXT("Unknown ShaderFrequency %d"), (int32_t)Frequency);
		break;
	}

	// Skip SF_
	int32_t Index = bIncludePrefix ? 0 : 3;
	String += Index;
	return String;
};
