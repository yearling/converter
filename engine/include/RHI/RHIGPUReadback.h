...
{
public:
	FRHIGPUMemoryUpdate(FRHIStagingBuffer *Staging, FVertexBufferRHIRef InBuffer, const std::string& RequestName)
	{
	...
	FRHIGPUMemoryUpdate(FVertexBufferRHIRef InBuffer, const std::string& RequestName)
	{
		GPUVertexBuffer = InBuffer;
...
public:
	...
	FRHIGPUMemoryReadback(FStagingBufferRHIRef Staging, FVertexBufferRHIRef InBuffer, const std::string& RequestName)
	{
		GPUVertexBuffer = InBuffer;
		StagingBuffer = Staging;
		Fence = RHICreateGPUFence(RequestName);
	...
	FRHIGPUMemoryReadback(FVertexBufferRHIRef InBuffer, const std::string& RequestName)
	{
		GPUVertexBuffer = InBuffer;
		StagingBuffer = RHICreateStagingBuffer();
		Fence = RHICreateGPUFence(RequestName);
