...
public:
	...
	{
		assert(!RTTasks.Num() && !Other.RTTasks.Num());
...
public:
	...
	inline FComputeFenceRHIRef CreateComputeFence(const FName& Name)
	{		
		return GDynamicRHI->RHICreateComputeFence(Name);
	}	

	inline FGPUFenceRHIRef CreateGPUFence(const FName& Name)
	{
