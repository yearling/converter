...
inline void DecodeRenderTargetMode(ESimpleRenderTargetMode Mode, ERenderTargetLoadAction& ColorLoadAction, ERenderTargetStoreAction& ColorStoreAction, ERenderTargetLoadAction& DepthLoadAction, ERenderTargetStoreAction& DepthStoreAction, ERenderTargetLoadAction& StencilLoadAction, ERenderTargetStoreAction& StencilStoreAction, FExclusiveDepthStencil DepthStencilUsage)
{
	...
	default:
		...
		ERROR_INFO("Using a ESimpleRenderTargetMode that wasn't decoded in DecodeRenderTargetMode [value = ", (int32)Mode);
	}
...
inline uint32 GetVertexCountForPrimitiveCount(uint32 NumPrimitives, uint32 PrimitiveType)
{
	...
	case PT_32_ControlPointPatchList: 
		...
		break;
	default:
		//UE_LOG(LogRHI, Fatal,TEXT("Unknown primitive type: %u"),PrimitiveType);
		ERROR_INFO("Unknown primtive type ", PrimitiveType);
	};
...
{
public:
	static void DumpResourceTransition(const std::string& ResourceName, const EResourceTransitionAccess TransitionType);
	
private:
	static void DumpTransitionForResourceHandler();

	//static TAutoConsoleVariable<FString> CVarDumpTransitionsForResource;
	static std::string  CVarDumpTransitionsForResource;
	
	//static FAutoConsoleVariableSink CVarDumpTransitionsForResourceSink;
	static  std::string  DumpTransitionForResource;
};
...
extern  void SetDepthBoundsTest(FRHICommandList& RHICmdList, float WorldSpaceDepthNear, float WorldSpaceDepthFar, const YMatrix& ProjectionMatrix);

/** Returns the value of the rhi.SyncInterval CVar. */
extern  uint32 RHIGetSyncInterval();
...
extern  void RHIGetPresentThresholds(float& OutTopPercent, float& OutBottomPercent);

/** Signals the completion of the specified task graph event when the given frame has flipped. */
extern  void RHICompleteGraphEventOnFlip(uint64 PresentIndex, FGraphEventRef Event);

extern  void RHIInitializeFlipTracking();
extern  void RHIShutdownFlipTracking();
...
struct FRHILockTracker
{
	...
	std::vector<FUnlockFenceParams> OutstandingUnlocks;
...
#endif
		OutstandingLocks.push_back(FLockParams(RHIBuffer, Buffer, Offset, SizeRHI, LockMode, bInDirectBufferWrite, bInCreateLock));
		TotalMemoryOutstanding += SizeRHI;
	}
	FORCEINLINE_DEBUGGABLE FLockParams Unlock(void* RHIBuffer, uint32 Offset=0)
	{
		for (int32 Index = 0; Index < OutstandingLocks.size(); Index++)
		{
	...
	{
		for (int32 Index = 0; Index < OutstandingUnlocks.size(); Index++)
		{
			if (OutstandingUnlocks[Index].RHIBuffer == RHIBuffer)
			{
				...
				OutstandingUnlocks.erase(OutstandingUnlocks.begin() + Index);
				return;
			}
		}
	}
	...
	{
		uint32 Count = OutstandingUnlocks.size();
...
extern  FRHILockTracker GRHILockTracker;
...
*/
/*
FRHIGPUMemoryReadback *RHIScheduleGPUMemoryReadback(FRHICommandList& CmdList, FVertexBufferRHIRef GPUBuffer, std::string RequestName)
{
...
FRHIGPUMemoryUpdate *RHIScheduleGPUMemoryUpdate(FRHICommandList& CmdList, FVertexBufferRHIRef GPUBuffer, std::string RequestName)
{
	FRHIStagingBuffer *StagingBuffer = RHICreateStagingBuffer();// new FRHIStagingBuffer();
