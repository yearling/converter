...
namespace ENamedThreads
{
	...
	{
		return FRenderThreadStatics::RenderThread.load(std::memory_order::memory_order_relaxed);
	}

	FORCEINLINE Type GetRenderThread_Local()
	{
		return FRenderThreadStatics::RenderThread_Local.load(std::memory_order::memory_order_relaxed);
	}

	FORCEINLINE void SetRenderThread(Type Thread)
	{
		FRenderThreadStatics::RenderThread.store(Thread, std::memory_order::memory_order_relaxed);
	}

	FORCEINLINE void SetRenderThread_Local(Type Thread)
	{
		FRenderThreadStatics::RenderThread_Local.store(Thread, std::memory_order::memory_order_relaxed);
	}
	...
	extern  int bHasBackgroundThreads;
	extern  int bHasHighPriorityThreads;

	inline Type GetThreadIndex(Type ThreadAndIndex)
	{
		return ((ThreadAndIndex & ThreadIndexMask) == AnyThread) ? AnyThread : Type(ThreadAndIndex & ThreadIndexMask);
	}

	inline int GetQueueIndex(Type ThreadAndIndex)
	{
		return (ThreadAndIndex & QueueIndexMask) >> QueueIndexShift;
	}

	inline int GetTaskPriority(Type ThreadAndIndex)
	{
		return (ThreadAndIndex & TaskPriorityMask) >> TaskPriorityShift;
	}

	inline int GetThreadPriorityIndex(Type ThreadAndIndex)
	{
	...
	inline Type SetPriorities(Type ThreadAndIndex, int PriorityIndex, bool bHiPri)
	{
		assert(
			!(ThreadAndIndex & ~ThreadIndexMask) && // not a thread index
...
protected:
	...
	**/
	...
	{
		checkThreadGraph(LifeStage.Increment() == int32_t(LS_Contructed));
	}
	/**
	...
	**/
	...
	static void LogPossiblyInvalidSubsequentsTask(const char* TaskName);
#endif
...
private:
	...
	*	@param CurrentThread; provides the index of the thread we are running on. This is handy for submitting new taks.
	**/
	inline void Execute(std::vector<FBaseGraphTask*>& NewTasks, ENamedThreads::Type CurrentThread)
	{
...
private:
	...
	*	Destroys myself.
	**/
	// NewTask is cache, in FTaskThreadBase::NewTasks
	virtual void ExecuteTask(std::vector<FBaseGraphTask*>& NewTasks, ENamedThreads::Type CurrentThread) final override
	{
		...
		if (TTask::GetSubsequentsMode() == ESubsequentsMode::TrackSubsequents)
		{
			FPlatformMisc::MemoryBarrier(); //todo memory barrier?
...
/** Task class for lambda based tasks. **/
class FFunctionGraphTaskNotUntil
{
public:
	/** Function to run **/
	std::function<void(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)> Function;
	/** Thread to run the function on **/
	const ENamedThreads::Type			DesiredThread;
...
public:
	...
	* @param InDesiredThread - Thread to run on
	**/
	FFunctionGraphTaskNotUntil(std::function<void(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)>&& InFunction, ENamedThreads::Type InDesiredThread)
		: Function(std::move(InFunction))
		, DesiredThread(InDesiredThread)
	{
	}
	...
	* @return completion handle for the new task
	**/
	static FGraphEventRef CreateAndDispatchWhenReady(std::function<void(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)> InFunction, const FGraphEventArray* InPrerequisites = NULL, ENamedThreads::Type InDesiredThread = ENamedThreads::AnyThread)
	{
		return TGraphTask<FFunctionGraphTaskNotUntil>::CreateTask(InPrerequisites).ConstructAndDispatchWhenReady(std::move(InFunction), InDesiredThread);
	}
	/**
	* Create a task and dispatch it when the prerequisites are complete
	* @param InTaskDelegate - delegate to execute when the prerequisites are complete
	...
	* @return completion handle for the new task
	**/
	static FGraphEventRef CreateAndDispatchWhenReady(std::function<void(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)> InFunction, const FGraphEventRef& InPrerequisite, ENamedThreads::Type InDesiredThread = ENamedThreads::AnyThread)
	{
		FGraphEventArray Prerequisites;
		assert(InPrerequisite.GetReference());
		Prerequisites.push_back(InPrerequisite);
